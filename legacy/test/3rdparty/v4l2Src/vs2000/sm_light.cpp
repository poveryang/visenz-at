...
int SM_LIGHT::init_light(void)
{
    ...
    fd_light = open(light_name, O_RDWR | O_NOCTTY | O_NONBLOCK);   //O_RDWR | O_NOCTTY
    if (fd_light < 0)
    {
        perror(light_name);
        return -1;
    } 
    set_fd_non_inheritable(fd_light);

    ret = light_config(115200, 8, 'N', 1);
	ret = getLightHWVersion();
	if (ret < 0 )
	{
		have_mcu = false;
	}
...
int SM_LIGHT::getCurrentVersion(void)
{
    int ret = 0;
    char command[20] = { "\0" };

    sprintf(command, "%s%d", light_hwversion, 3);
    Lihgt_Send(command, strlen(command)+1);
    memset(command, 0, sizeof(command));
    ret = Light_Recv(command, 20, 10);
	if (ret > 0)
