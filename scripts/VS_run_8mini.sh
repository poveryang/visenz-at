...
DEV_IP=169.254.19.140

# Clean local files
SAVE_DIR=./AT_RES/VS800/$(date +%m%d%H%M)
echo "$SAVE_DIR" && mkdir "$SAVE_DIR"

# Scp execute file and lib to remote device
scp -O ./release/VS/imx8mini/lib/libAT.so root@$DEV_IP:/tmp
scp -O ./release/VS/imx8mini/bin/AT_TEST root@$DEV_IP:/tmp
