...
DEV_IP=169.254.19.137

# Clean local files
SAVE_DIR=./AT_RES/VN800/$(date +%m%d%H%M)
echo "$SAVE_DIR" && mkdir "$SAVE_DIR"

# Scp execute file and lib to remote device
scp -O ./release/VN/imx8mini/lib/libAT.so root@$DEV_IP:/tmp
scp -O ./release/VN/imx8mini/bin/AT_TEST root@$DEV_IP:/tmp
