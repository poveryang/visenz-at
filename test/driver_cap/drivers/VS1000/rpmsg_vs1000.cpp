...
int Rpmsg::setLightAim(int status)
{
    ...
    char command[20] = { 0 };
    sprintf(command, "%s9%d0", light_head,status);
...
int Rpmsg::setLightEnable(int num, int state)
{
    int ret = 0;
...
int Rpmsg::setPlcStatus(int num, bool status)
{
    int ret = 0;

    char command[20] = { 0 };
    sprintf(command, "%s%d%d", plc_head, num, status);
    ret = Rpmsg_SendDate(command, strlen(command));

    return ret >= 0 ? 0 : -1;
}

int Rpmsg::setLedStatus(int num, bool status)
{
    int ret = 0;

    char command[20] = { 0 };
    sprintf(command, "%s%d%d", led_head, num, status);
    ret = Rpmsg_SendDate(command, strlen(command));
