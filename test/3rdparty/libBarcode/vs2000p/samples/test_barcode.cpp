...
int main(int argc, char **argv)
{
    ...
    std::string help_info = R"""(
        -d directory path
        -t barcode type, can be [1D, 2D, CODE_39, CODE_93, CODE_128, EAN_8, EAN_13, ITF, UPC_A, UPC_E, UPC_EAN_EXTENSION, QR, DM, PHARMA_CODE, Pdf417], multi-type input is supported
    ...
    while (loop_counts != 0)
    {
        ...
        for (int i = 0; i < file_list.size(); i++)
        {
            ...
            for (auto &result : output.results)
            {
                ...
                cv::putText(draw,
                            "idx:" + std::to_string(static_cast<int>(idx / 2)),
            ...
            cv::putText(draw,
                        "decode_num: " +
                            std::to_string(static_cast<int>(idx / 2)),
