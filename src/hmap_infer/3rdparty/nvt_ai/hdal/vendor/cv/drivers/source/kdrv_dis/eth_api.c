...
int exam_dis_eth_ipe_cb(PDIS_MODULE_INFO pmodule_info, unsigned char argc, char **p_argv)
{
	...
	#else
    static CTL_IPP_ISP_ETH eth_set_param = {ENABLE, 0, 0, 0, 1, {0, 0}, {0, 0}, 28, 48, 128, {0, 0}};
	#endif
	UINT32 buf_size = 0;	
	...
	for (i = 0; i < ETH_BUFFER_NUM; i++) {
		#ifdef SIE_ETH
		buf_size = dis_eth_util_ethsize(&w, &h, g_eth_param.eth_info.out_bit_sel, g_eth_param.eth_info.out_sel, &out_lofs);
		#else
		buf_size = dis_eth_util_ethsize(&w, &h, g_eth_param.out_bit_sel, g_eth_param.out_sel, &out_lofs);
		#endif
...
int vendor_dis_eth_ipe_cb(PDIS_MODULE_INFO pmodule_info, unsigned char argc, char **p_argv)
{
    ...
    UINT32 h = 1080;
	for (i = 0; i < ETH_BUFFER_NUM; i++) {
		buf_size = dis_eth_util_ethsize(&w, &h, 0, 0, &out_lofs);
		len = nvt_fmem_mem_info_init(&eth_bufinfo[i], NVT_FMEM_ALLOC_CACHE, buf_size, NULL);
		if (len >= 0) {
			hdl_buf = nvtmem_alloc_buffer(&eth_bufinfo[i]);
		}
