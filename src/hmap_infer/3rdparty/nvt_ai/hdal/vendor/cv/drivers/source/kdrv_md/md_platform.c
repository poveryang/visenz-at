...
ER md_platform_set_clk_rate(UINT32 uiClock)
{
    ...
    if (uiClock >= MDBC_CLK_PLL13) {
		if (uiClock > MDBC_CLK_PLL13)DBG_WRN("MDBC: Input frequency %d round to 360\r\n", uiClock);
        g_uiMdbcTrueClock = 360;
		parent_clk = clk_get(NULL, "pll13");
    }
    else if (uiClock >= MDBC_CLK_320) {
		if (uiClock > MDBC_CLK_320)DBG_WRN("MDBC: input frequency %d round to 320MHz\r\n", uiClock);
		g_uiMdbcTrueClock = 320;
		parent_clk = clk_get(NULL, "pllf320");
	}
	else if (uiClock >= MDBC_CLK_240) {
		if (uiClock > MDBC_CLK_240)DBG_WRN("MDBC: input frequency %d round to 240MHz\r\n", uiClock);
	...
	if (uiClock >= 360) {
		if (uiClock > 360)DBG_ERR("Input frequency %d round to 360\r\n", uiClock);
		g_uiMdbcTrueClock = 360;
		md_true_clock = PLL_CLKSEL_MDBC_PLL13;
		if (pll_getPLLEn(PLL_ID_13) == FALSE) {
			pll_setPLLEn(PLL_ID_13, TRUE);
		}
	} else if (uiClock >= 320) {
		if (uiClock > 320)DBG_ERR("Input frequency %d round to 320\r\n", uiClock);
		g_uiMdbcTrueClock = 320;
		md_true_clock = PLL_CLKSEL_MDBC_320;
...
UINT32 md_platform_pa2va_remap(UINT32 pa, UINT32 sz)
{
#if defined(__FREERTOS)
	return pa;
#else
...
VOID md_platform_pa2va_unmap(UINT32 va, UINT32 pa)
{
#if defined(__FREERTOS)
	return;
#else
