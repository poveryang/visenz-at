#ifndef _KDRV_AI_DEBUG_H_
#define _KDRV_AI_DEBUG_H_
...
extern unsigned int kdrv_ai_debug_level;

#define nvt_dbg(level, fmt, args...) do { \
	if (unlikely(NVT_DBG_##level <= kdrv_ai_debug_level)) { \
		vk_pr_warn(fmt, ##args); \
	} \
} while (0)
