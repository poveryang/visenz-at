...
static int m_ai_preload = 0;
static int m_ai_preload_init = 0;
...
int nvt_ai_drv_init(AI_INFO *pmodule_info)
{
		...
		if (do_ai_init == 1) {
			...
			for (i = 0; i < MODULE_IRQ_NUM; i++) {
				if (request_irq(pmodule_info->iinterrupt_id[i], nvt_ai_drv_isr, IRQF_TRIGGER_HIGH, "AI_INT", pmodule_info)) {
					DBG_ERR("failed to register an IRQ Int:%d, KDRV_AI_MINI_FOR_FASTBOOT (%d), is_ai_enable_dtsi (%d), do_ai_init (%d), kdrv_ai_is_fastboot(%d)\n", 
							pmodule_info->iinterrupt_id[i], KDRV_AI_MINI_FOR_FASTBOOT, is_ai_enable_dtsi, do_ai_init, kdrv_ai_is_fastboot());
					DBG_ERR("[kdrv_ai] please check builtin kdrv ai is opened or not\r\n");
					err = -ENODEV;
					goto FAIL_FREE_IRQ;
				}
			}
		} else {
			DBG_ERR("[kdrv_ai] no request_irq! KDRV_AI_MINI_FOR_FASTBOOT (%d), is_ai_enable_dtsi (%d), do_ai_init (%d), kdrv_ai_is_fastboot(%d)\r\n", KDRV_AI_MINI_FOR_FASTBOOT, is_ai_enable_dtsi, do_ai_init, kdrv_builtin_is_fastboot());
			DBG_ERR("[kdrv_ai] please check ai_enable in dtsi, ai_enable should be 1\r\n");
		}
	} else {
    ...
    if (ret >= 0) {
		...
		DBG_ERR("init buffer fail\n");
		return -EINVAL;
	}
#endif
#endif
	//========
	//DBG_IND("ai set engine resource...done\r\n");
	return err;
...
#if (LL_BUF_FROM_USR == 0)
	if (g_buf_handle == NULL) {
		DBG_ERR("free buf: handle is null!\n");
		return -1;
	} else {
...
int nvt_ai_drv_ioctl(unsigned char if_id, AI_INFO *pmodule_info, unsigned int cmd, unsigned long argc)
{
...
#endif
#endif
	AI_DRV_RUN_INFO ai_run_info = {0};
	AI_DRV_USR_DBG_INFO ai_dbg_info = {0};
	...
	case AI_IOC_ENG_INIT:
		...
		if (err != 0) {
            err = -EFAULT;
            DBG_ERR("AI_DRV: Error, nue_init failed.\r\n");
            goto AI_IOC_ENG_INIT_EXIT;
        }
		err = cnn_init(0);
		if (err != 0) {
            err = -EFAULT;
            DBG_ERR("AI_DRV: Error, cnn_init failed.\r\n");
            goto AI_IOC_ENG_INIT_EXIT;
        }
		err = nue2_init();
		if (err != 0) {
            err = -EFAULT;
            DBG_ERR("AI_DRV: Error, nue2_init failed.\r\n");
            goto AI_IOC_ENG_INIT_EXIT;
        }
...
AI_IOC_ENG_INIT_EXIT:
	...
	case AI_IOC_ENG_UNINIT:
		...
		if (err != 0) {
            err = -EFAULT;
            DBG_ERR("AI_DRV: Error, nue_uninit failed.\r\n");
            goto AI_IOC_ENG_UNINIT_EXIT;
        }
		err = cnn_uninit(0);
		if (err != 0) {
            err = -EFAULT;
            DBG_ERR("AI_DRV: Error, cnn_uninit failed.\r\n");
            goto AI_IOC_ENG_UNINIT_EXIT;
        }
		err = nue2_uninit();
		if (err != 0) {
            err = -EFAULT;
            DBG_ERR("AI_DRV: Error, nue2_uninit failed.\r\n");
            goto AI_IOC_ENG_UNINIT_EXIT;
        }
...
AI_IOC_CNN2_UNINIT_EXIT:
	...
	case AI_IOC_OPEN:
		...
		if (er_return != E_OK) {
            err = -EFAULT;
            DBG_ERR("AI_DRV: Error, kdrv_ai_open failed.\r\n");
...
AI_IOC_OPEN_EXIT:
	...
	case AI_IOC_CLOSE:
		...
		if (er_return != E_OK) {
            err = -EFAULT;
            DBG_ERR("AI_DRV: Error, kdrv_ai_close failed.\r\n");
...
AI_IOC_CLOSE_EXIT:
	...
	case AI_IOC_OPENCFG:
		...
		if (er_return != E_OK) {
            err = -EFAULT;
            DBG_ERR("AI_DRV: Error, kdrv_ai_set failed.\r\n");
...
AI_IOC_SET_LL_EXIT:
	...
	case AI_IOC_GET_MODE:
		...
		if (er_return != E_OK) {
            DBG_ERR("AI_DRV: Error, kdrv_ai_get failed.\r\n");
...
AI_IOC_GET_LL_EXIT:
	...
	case AI_IOC_TRIGGER:
		...
		if (er_return != E_OK) {
            err = -EFAULT;
            DBG_ERR("AI_DRV: Error, kdrv_ai_trigger failed.\r\n");
...
AI_IOC_TRIGGER_EXIT:
	...
	case AI_IOC_WAITDONE:
		...
		if (er_return != E_OK) {
            err = -EFAULT;
            DBG_ERR("AI_DRV: Error, kdrv_ai_waitdone failed.\r\n");
...
AI_IOC_WAITDONE_EXIT:
	...
	case AI_IOC_RESET:
		...
		if (er_return != E_OK) {
            err = -EFAULT;
            DBG_ERR("AI_DRV: Error, kdrv_ai_reset failed.\r\n");
...
AI_IOC_RESET_EXIT:
	...
	case AI_IOC_SET_LL_USR_INFO: {
		...
		if (er_return != E_OK) {
            err = -EFAULT;
            DBG_ERR("AI_DRV: Error, kdrv_ai_link_ll_update_addr failed.\r\n");
...
AI_IOC_SET_LL_USR_INFO_EXIT:
	...
	case AI_IOC_UNINIT_LINK_INFO: {
		...
		if (er_return != E_OK) {
            err = -EFAULT;
            DBG_ERR("AI_DRV: Error, kdrv_ai_link_uninit failed.\r\n");
...
AI_IOC_UNINIT_LINK_INFO_EXIT:
	...
	case NUE_IOC_RUN_FC: {
		...
		kdrv_ai_tran_nue_fc_parm(&input_parm, &g_nue_drv_parm, KDRV_AI_FC_FULLY_EN);

		g_nue_drv_parm.dmaio_addr.drv_dma_not_sync[0] = 1;
		g_nue_drv_parm.dmaio_addr.drv_dma_not_sync[2] = 1;
		g_nue_drv_parm.dmaio_addr.drv_dma_not_sync[3] = 1;
		g_nue_drv_parm.dmaio_addr.drv_dma_not_sync[4] = 1;
		g_nue_drv_parm.dmaio_addr.drv_dma_not_sync[6] = 1;

#if AI_IOREMAP_IN_KERNEL
		// map va
		// input
		ioremap_size = input_parm.fc_ker.weight_w;
		input_pa = g_nue_drv_parm.dmaio_addr.addrin0;
		g_nue_drv_parm.dmaio_addr.addrin0 = kdrv_ai_pa2va_remap_wo_sync(g_nue_drv_parm.dmaio_addr.addrin0, ioremap_size);
		// output
		ioremap_size = 4*input_parm.fc_ker.weight_h;
		output_pa = input_parm.out_interm_addr;
		input_parm.out_interm_addr = kdrv_ai_pa2va_remap_wo_sync(input_parm.out_interm_addr, ioremap_size);
		// weight
		ioremap_size = input_parm.fc_ker.weight_w*input_parm.fc_ker.weight_h;
		weight_pa = input_parm.fc_ker.weight_addr;
		input_parm.fc_ker.weight_addr = kdrv_ai_pa2va_remap_wo_sync(input_parm.fc_ker.weight_addr, ioremap_size);
#endif
		run_time = (g_nue_drv_parm.insvm_size.sv_h / 256) + (((g_nue_drv_parm.insvm_size.sv_h % 256) > 0)?1:0);
		tmp_h    = input_parm.fc_ker.weight_h;
		er_return = nue_open(NULL);
		if (er_return != E_OK) {
			err = -EFAULT;
			DBG_ERR("AI_DRV: Error, nue_open failed.\r\n");
            goto NUE_IOC_RUN_FC_EXIT;
		}
		for (run_idx = 0; run_idx < run_time; run_idx++) {
			g_nue_drv_parm.insvm_size.sv_h = (tmp_h < 256)?tmp_h:256;
			g_nue_drv_parm.dmaio_addr.addr_out  = input_parm.out_interm_addr + run_idx*4*256;
			g_nue_drv_parm.dmaio_addr.addrin_sv = input_parm.fc_ker.weight_addr + run_idx*g_nue_drv_parm.insvm_size.sv_w*256;
			g_nue_drv_parm.svm_parm.fcd_parm.fcd_enc_bit_length = input_parm.fc_ker.weight_w*g_nue_drv_parm.insvm_size.sv_h*8;
...
#else
			if (nue_setmode(NUE_OPMODE_USERDEFINE, &g_nue_drv_parm) == E_OK) {
				er_return = nue_start();
				if (er_return != E_OK) {
					err = -EFAULT;
					DBG_ERR("AI_DRV: Error, nue_start failed.\r\n");
			...
			if (er_return != E_OK) {
				err = -EFAULT;
				DBG_ERR("AI_DRV: Error, nue_pause failed.\r\n");
				goto NUE_IOC_RUN_FC_1_EXIT;
			}
#endif
			tmp_h -= g_nue_drv_parm.insvm_size.sv_h;
		}
...
NUE_IOC_RUN_FC_1_EXIT:
		...
		if (er_return != E_OK) {
            err = -EFAULT;
            DBG_ERR("AI_DRV: Error, nue_close failed.\r\n");
            goto NUE_IOC_RUN_FC_EXIT;
        }
...
NUE_IOC_RUN_FC_EXIT:
		...
		// unmap va
		// input
		kdrv_ai_pa2va_unmap(g_nue_drv_parm.dmaio_addr.addrin0, input_pa);
		// output
		kdrv_ai_pa2va_unmap(input_parm.out_interm_addr, output_pa);
		// weight
		kdrv_ai_pa2va_unmap(input_parm.fc_ker.weight_addr, weight_pa);
#endif
	}
		break;
	case NUE_IOC_INIT:
...
NUE_IOC_UNINIT_EXIT:
	...
	case NUE2_IOC_RUN: {
		...
		if (er_return != E_OK) {
            err = -EFAULT;
            DBG_ERR("AI_DRV: Error, kdrv_ai_tran_nue2_preproc_parm failed.\r\n");
		...
		if (er_return != E_OK) {
            err = -EFAULT;
            DBG_ERR("AI_DRV: Error, nue2_open failed.\r\n");
            goto NUE2_IOC_RUN_EXIT;
        }
#if (KDRV_AI_MINI_FOR_FASTBOOT == 2 || KDRV_AI_MINI_FOR_FASTBOOT == 1)
		DBG_ERR("AI_FASTBOOT: Error, NUE2_IOC_RUN can't be called at fastboot.\n");
#else
		if (nue2_setmode(NUE2_OPMODE_USERDEFINE, &g_nue2_parm) == E_OK) {
			er_return = nue2_start();
			if (er_return != E_OK) {
				err = -EFAULT;
				DBG_ERR("AI_DRV: Error, nue2_start failed.\r\n");
				goto NUE2_IOC_RUN_EXIT;
			}
...
NUE2_IOC_RUN_EXIT:
	...
	case NUE2_IOC_RUN2: {
		KDRV_AI_PREPROC_INFO ai_preproc_info = {0};

		if (copy_from_user(&ai_preproc_info, (void __user *)argc, sizeof(KDRV_AI_PREPROC_INFO))) {
			err = -EFAULT;
			goto exit;
		}
#if (KDRV_AI_MINI_FOR_FASTBOOT == 2 || KDRV_AI_MINI_FOR_FASTBOOT == 1)
        DBG_ERR("AI_FASTBOOT: Error, NUE2_IOC_RUN2 can't be called at fastboot.\n");
#else
		err = kdrv_ai_preproc_run(&ai_preproc_info);
#endif
	}
		break;

	case NUE2_IOC_GET_PARM_SIZE: {
		UINT32 parm_size = sizeof(NUE2_PARM);
		err = copy_to_user((void __user *)argc, &parm_size, sizeof(UINT32));
	}
		break;
	...
	case NUE2_IOC_DONE:
		...
		if (er_return != E_OK) {
            err = -EFAULT;
            DBG_ERR("AI_DRV: Error, nue2_pause failed.\r\n");
            goto NUE2_IOC_DONE_EXIT;
        }
		er_return = nue2_close();
		if (er_return != E_OK) {
            err = -EFAULT;
            DBG_ERR("AI_DRV: Error, nue2_close failed.\r\n");
            goto NUE2_IOC_DONE_EXIT;
        }
...
NUE2_IOC_UNINIT_EXIT:
	...
	case AI_IOC_DMA_ABORT:
		...
		if (er_return != E_OK) {
            err = -EFAULT;
            DBG_ERR("AI_DRV: Error, kdrv_ai_dma_abort failed.\r\n");
...
AI_IOC_DMA_ABORT_EXIT:
	...
	case AI_IOC_GET_VER: {
		...
		break;
	case AI_IOC_RUN_LL: {
		KDRV_AI_TRIG_MODE trig_mode = AI_TRIG_MODE_LL;

		if (copy_from_user(&ai_run_info, (void __user *)argc, sizeof(AI_DRV_RUN_INFO))) {
			err = -EFAULT;
			goto exit;
		}
		ai_eng = ai_run_info.engine;
		...
		er_return = kdrv_ai_set(id, KDRV_AI_PARAM_MODE_INFO, &trig_mode);
		if (er_return != E_OK) {
            err = -EFAULT;
            DBG_ERR("AI_DRV: Error, kdrv_ai_set failed.\r\n");
            goto exit;
		...
		// set ll info
		if (!nvt_ai_drv_is_mem_map(ai_run_info.net_id)) {
			DBG_ERR("memory should be mapped before ll parms set\r\n");
			goto exit;
		}
		ai_run_info.info.p_head = (KDRV_AI_LL_HEAD*)nvt_ai_drv_user2kerl_va((unsigned int)ai_run_info.info.p_head, ai_run_info.net_id);
		if ((int)ai_run_info.info.p_head == 0) {
			goto exit;
		}

		er_return = kdrv_ai_set(id, KDRV_AI_PARAM_LL_INFO, &ai_run_info.info);
		if (er_return != E_OK) {
            err = -EFAULT;
            DBG_ERR("AI_DRV: Error, kdrv_ai_set failed.\r\n");
            goto exit;
		...
		// trigger
		er_return = kdrv_ai_trigger(id, &ai_run_info.trig_parm, NULL, NULL);
		if (er_return != E_OK) {
            err = -EFAULT;
            DBG_ERR("AI_DRV: Error, kdrv_ai_trigger failed.\r\n");
            goto exit;
	...
	case AI_IOC_SET_DBG_INFO:
		if (copy_from_user(&ai_dbg_info, (void __user *)argc, sizeof(AI_DRV_USR_DBG_INFO))) {
			err = -EFAULT;
			goto exit;
		}
		cnn_set_perf_en(ai_dbg_info.cnn1_info.eng_prof_en, 0);
		cnn_set_perf_en(ai_dbg_info.cnn2_info.eng_prof_en, 1);
		nue_set_perf_en(ai_dbg_info.nue_info.eng_prof_en);
		nue2_set_perf_en(ai_dbg_info.nue2_info.eng_prof_en);
		break;

	case AI_IOC_GET_ENG_INFO: {
		AI_DRV_USR_ENG_INFO query_info = {0};

		query_info.cnn1_info.eng_time = cnn_get_eng_time(0);
		query_info.cnn1_info.eng_util = cnn_get_eng_util(0);
		query_info.cnn2_info.eng_time = cnn_get_eng_time(1);
		query_info.cnn2_info.eng_util = cnn_get_eng_util(1);
		query_info.nue_info.eng_time  = nue_get_eng_time();
		query_info.nue_info.eng_util  = nue_get_eng_util();
		query_info.nue2_info.eng_time = nue2_get_eng_time();
		query_info.nue2_info.eng_util = nue2_get_eng_util();
		err = copy_to_user((void __user *)argc, &query_info, sizeof(AI_DRV_USR_ENG_INFO));

	}
		break;
#if KDRV_AI_SUPPORT_NUELL
	case NUE_IOC_SET_LL_CMD: {
		KDRV_AI_FC_INFO fc_info = {0};

		if (copy_from_user(&fc_info, (void __user *)argc, sizeof(KDRV_AI_FC_INFO))) {
			err = -EFAULT;
			goto exit;
		}

		// construct ll cmd
		fc_info.input_parm_va = 0;
		fc_info.eng_parm_va = 0;
		kdrv_ai_construct_nue_fc_cmd(&fc_info);
	}
		break;

	case NUE_IOC_RUN_FC_LL: {
		KDRV_AI_FC_INFO fc_info = {0};
		...
		// run fc
		kdrv_ai_nue_ll_trig(fc_info.cmd_pa);
		kdrv_ai_nue_ll_done(NULL);
	...
	case NUE_IOC_GET_PARM_SIZE: {
		UINT32 parm_size = sizeof(NUE_PARM);
		err = copy_to_user((void __user *)argc, &parm_size, sizeof(UINT32));
	}
		break;
#endif
...
int kdrv_ai_is_preload_model(void)
{
	if (m_ai_preload_init) {
		return m_ai_preload;
	}
	{
		char *path = "/ai-memory/ai_model";
		struct device_node* dt_node;
...
#endif
EXPORT_SYMBOL(kdrv_ai_is_fastboot);
EXPORT_SYMBOL(kdrv_ai_is_preload_model);
