...
VOID nue_pt_loop_frameend(VOID)
{
	...
	while (((nue_int_status & NUE_INT_ROUTEND) != NUE_INT_ROUTEND)  && (dur_time < time_limit) ) {
        ...
        count++;
		if ((count % 100) == 0) {
			DBG_WRN("NUE loop delay count=%d\r\n", count);
		}
        if (count > 1000) {
            DBG_WRN("NUE frame end wait too long(timeout).\r\n");
			er_return = nue_pause();
			if (er_return != E_OK) {
				DBG_ERR("NUE Error to do nue_pause. sts=%d\r\n", (int) er_return);
			}
            er_return = nue_close();
			if (er_return != E_OK) {
                DBG_ERR("NUE Error to do nue_close. sts=%d\r\n", (int) er_return);
            }
            er_return = nue_open(NULL);
			if (er_return != E_OK) {
                DBG_ERR("NUE Error to do nue_open. sts=%d\r\n", (int) er_return);
            }
            break;
        }
#else
...
ER nue_pt_set_clock_rate(UINT32 clock)
{
	...
	if (IS_ERR(pclk)) {
		DBG_ERR("%s: get source fix480m fail\r\n", __func__);
	}
