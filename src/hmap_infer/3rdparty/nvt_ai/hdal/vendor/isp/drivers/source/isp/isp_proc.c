...
static INT32 isp_proc_help_show(struct seq_file *sfile, void *v)
{
	...
	seq_printf(sfile, " | 0x%8X = LV           | 0x%8X = CA_ENABLE    | 0x%8X = LA_ENABLE    | 0x%8X = CA_TH        |\r\n", ISP_DBG_SYNC_LV, ISP_DBG_SYNC_CA_ENABLE, ISP_DBG_SYNC_LA_ENABLE, ISP_DBG_SYNC_CA_TH);
	...
	seq_printf(sfile, " | 0x%8X = ENC_PARAM    | 0x%8X = SEN_REG      | 0x%8X = SEN_EXPT     | 0x%8X = SEN_GAIN     | \r\n", ISP_BYPASS_ENC_PARAM, ISP_BYPASS_SEN_REG, ISP_BYPASS_SEN_EXPT, ISP_BYPASS_SEN_GAIN);
	seq_printf(sfile, " | 0x%8X = SEN_DIR      | 0x%8X = SEN_SLEEP    | 0x%8X = SEN_WAKEUP   | \r\n", ISP_BYPASS_SEN_DIR, ISP_BYPASS_SEN_SLEEP, ISP_BYPASS_SEN_WAKEUP);
	seq_printf(sfile, "---------------------------------------------------------------------\r\n");
	...
	seq_printf(sfile, "Ex: 'echo w ioc 0x0000 > /proc/hdal/vendor/isp/cmd' \r\n");
	seq_printf(sfile, "Ex: 'echo w ioc 0x0080 > /proc/hdal/vendor/isp/cmd' \r\n");
	seq_printf(sfile, "Ex: 'echo w ioc 0x800000 > /proc/hdal/vendor/isp/cmd' \r\n");
