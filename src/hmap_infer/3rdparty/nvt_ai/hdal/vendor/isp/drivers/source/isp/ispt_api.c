...
static void ispt_api_get_ca_enable(UINT32 addr);
static void ispt_api_get_la_enable(UINT32 addr);
...
static void ispt_api_set_ca_enable(UINT32 addr);
static void ispt_api_set_la_enable(UINT32 addr);
...
static void ispt_api_set_d_gain(UINT32 addr)
{
	...
	if (nvt_get_chip_id() == CHIP_NA51084) {
		max_d_gain = 0x7FFF;  // 256x
	} else {
		max_d_gain = 0x3FF;   // 8x
	}

	if (data->gain > max_d_gain) {
		PRINT_ISP_WRN(isp_dbg_get_dbg_mode(data->id) & ISP_DBG_WRN_MSG, "d_gain(%d) out of range, force to %d\r\n", data->gain, max_d_gain);
		data->gain = max_d_gain;
	}
