...
static int nvt_ctl_sie_proc_buf_show(struct seq_file *sfile, void *v)
{
	...
	for (id = CTL_SIE_ID_1; id < CTL_SIE_MAX_SUPPORT_ID; id++) {
		ctl_sie_dbg_set_msg_type(id, CTL_SIE_DBG_MSG_ISR_IOCTL, 1, 0, 0);
	}

	pdrv_info_data->info_seq_file = sfile;
	vos_util_delay_ms(1000);
	ctl_sie_dbg_dump_isr_ioctl(nvt_ctl_sie_seq_printf);

	for (id = CTL_SIE_ID_1; id < CTL_SIE_MAX_SUPPORT_ID; id++) {
		ctl_sie_dbg_set_msg_type(id, CTL_SIE_DBG_MSG_ISR_IOCTL, 0, 0, 0);
	}
	pdrv_info_data->info_seq_file = NULL;
	return 0;
}
...
static struct file_operations proc_buf_fops = {
	.owner  = THIS_MODULE,
	.open   = nvt_ctl_sie_proc_buf_open,
	.release = single_release,
	.read   = seq_read,
	.llseek = seq_lseek,
};
