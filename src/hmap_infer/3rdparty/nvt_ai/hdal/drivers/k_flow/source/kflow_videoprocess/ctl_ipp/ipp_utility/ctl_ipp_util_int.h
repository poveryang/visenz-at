...
*/
typedef enum {
	CTL_IPP_RHE_FUNC_MODE_LINEAR              = 0,    ///< linear mode
	CTL_IPP_RHE_FUNC_MODE_DFS                 = 1,    ///< linear mode with DFS
	CTL_IPP_RHE_FUNC_MODE_PHDR                = 2,    ///< pseudo-HDR mode
	CTL_IPP_RHE_FUNC_MODE_SHDR                = 3,    ///< sensor-HDR mode
	CTL_IPP_RHE_FUNC_MODE_SHDR_WITH_PHDR      = 4,    ///< sensor-HDR mode
	CTL_IPP_RHE_FUNC_MODE_MAX,
} CTL_IPP_RHE_FUNC_MODE;
...
typedef struct {
	...
	UINT32 ui_fact_h;
	UINT32 ui_fact_v;
	UINT32 ui_pofs_h_int;
	UINT32 ui_pofs_h_dec;
	UINT32 ui_pofs_v_int;
	UINT32 ui_pofs_v_dec;
	UINT32 ui_nofs_h_int;
	UINT32 ui_nofs_h_dec;
	UINT32 ui_nofs_v_int;
	UINT32 ui_nofs_v_dec;
} CTL_IPP_RHE_SCL_INFO;

typedef struct {
	CTL_IPP_RHE_FUNC_MODE func_mode;

	/* input image info
		addr[0~3] support most 4 frame input
	*/
	UINT32 in_addr[4];
	UINT32 in_lofs;
	USIZE in_size;
	...
	*/
	UINT32 subin_addr[4];
	UINT32 subin_lofs;
	USIZE subin_img_size;
	USIZE subin_blk_size;
	...
	/* cadj input/output info */
	BOOL cadj_out_enable;
	UINT32 cadj_out_addr;
	UINT32 cadj_in_addr;
...
typedef struct {
	...
	*/
	UINT32 strp_h_arr[CTL_IPP_DCE_STRP_NUM_MAX];
...
typedef struct {
	...
	*/
	...
	/* defog parameter */
	BOOL defog_enable;
	UINT32 defog_in_addr;
	UINT32 defog_out_addr;
...
typedef struct {
	...
	BOOL flip_enable;
	VDO_PXLFMT fmt;
	UINT32 addr[3];	/* virtual address */
	UINT32 pa[3];	/* physical address */
	UINT32 lofs[3];
	USIZE size;
	URECT crp_window;
	UINT32 one_buf_mode_enable;
	UINT32 md_enable;
	UINT32 md_addr;
	UINT32 h_align;
...
typedef struct {
	...
	BOOL tplnr_enable;
	BOOL tplnr_out_ms_roi_enable;

	UINT32 tplnr_in_ref_path;
	UINT32 tplnr_in_ref_addr[3];	/* virtual address */
	UINT32 tplnr_in_ref_pa[3];		/* physical address */
	UINT32 tplnr_in_ref_lofs[3];
	BOOL tplnr_in_ref_flip_enable;

	BOOL tplnr_in_dec_enable;
...
typedef struct {
	...
	CTL_IPP_IFE2_CTRL ife2_ctrl;
	UINT32 ctl_frm_sta_count;
