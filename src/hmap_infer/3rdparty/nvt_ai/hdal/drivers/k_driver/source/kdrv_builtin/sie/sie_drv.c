...
irqreturn_t nvt_sie_drv_isr4(int irq, void *devid);
irqreturn_t nvt_sie_drv_isr5(int irq, void *devid);
/*===========================================================================*/
/* Define                                                                    */
/*===========================================================================*/
...
int nvt_sie_drv_init(SIE_MODULE_INFO* pmodule_info, unsigned char ucIF)
{
	...
	if(request_irq(pmodule_info->iinterrupt_id[3], nvt_sie_drv_isr4, IRQF_TRIGGER_HIGH, "SIE4_INT", pmodule_info)) {
		nvt_dbg(ERR, "failed to register an IRQ IF:%d Int:%d\n", 3, pmodule_info->iinterrupt_id[3]);
		iRet = -ENODEV;
		goto FAIL_FREE_IRQ;
	}

	if(request_irq(pmodule_info->iinterrupt_id[4], nvt_sie_drv_isr5, IRQF_TRIGGER_HIGH, "SIE5_INT", pmodule_info)) {
		nvt_dbg(ERR, "failed to register an IRQ IF:%d Int:%d\n", 4, pmodule_info->iinterrupt_id[4]);
		iRet = -ENODEV;
		goto FAIL_FREE_IRQ;
	}
	...
	sie_platform_prepare_clk(SIE_ENGINE_ID_4);
	sie_platform_prepare_clk(SIE_ENGINE_ID_5);
...
int nvt_sie_drv_remove(SIE_MODULE_INFO* pmodule_info)
{
	...
	free_irq(pmodule_info->iinterrupt_id[3], pmodule_info);
	free_irq(pmodule_info->iinterrupt_id[4], pmodule_info);
