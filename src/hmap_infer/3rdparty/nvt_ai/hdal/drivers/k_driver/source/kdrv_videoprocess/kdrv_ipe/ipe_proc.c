...
typedef struct proc_cmd {
	...
	int (*execute)(PIPE_INFO pdrv, unsigned char argc, char **argv);
} PROC_CMD, *PPROC_CMD;

//============================================================================
// Global variable
//============================================================================
static PIPE_DRV_INFO pdrv_info_data;
...
static PROC_CMD cmd_read_list[] = {
	...
	{ "reg",            nvt_ipe_api_read_reg           }, // read reg
	{ "dump",           nvt_ipe_api_dump               }  // dump ipe info
#endif
};
...
static PROC_CMD cmd_write_list[] = {
	// keyword          function name
#if (__IPE_DBG_CMD__ == 1)
	{ "reg",            nvt_ipe_api_write_reg       }, // write value to hw
	{ "dbg",            nvt_ipe_set_dbg_cmd,        },
	{ "iq",             nvt_ipe_api_iq_test         }, // write value to hw
	{ "pattern",        nvt_kdrv_ipe_api_test       }, // read file from sd:
#endif

#if !defined (CONFIG_NVT_SMALL_HDAL)
	{ "ipe_hw_pwsv",    nvt_ipe_hw_power_saving        },
	{ "ipe_fw_pwsv",    nvt_ipe_fw_power_saving        },
#endif
};

#define NUM_OF_WRITE_CMD (sizeof(cmd_write_list) / sizeof(PROC_CMD))

static int nvt_ipe_proc_cmd_show(struct seq_file *sfile, void *v)
{
	nvt_dbg(IND, "\n");
	return 0;
}

static int nvt_ipe_proc_cmd_open(struct inode *inode, struct file *file)
{
	nvt_dbg(IND, "\n");
	return single_open(file, nvt_ipe_proc_cmd_show, &pdrv_info_data->module_info);
}

static ssize_t nvt_ipe_proc_cmd_write(struct file *file, const char __user *buf, size_t size, loff_t *off)
{
	int len = size;
	int ret = -EINVAL;
...
static struct file_operations proc_cmd_fops = {
	.owner   = THIS_MODULE,
	.open    = nvt_ipe_proc_cmd_open,
	.read    = seq_read,
	.llseek  = seq_lseek,
	.release = single_release,
	.write   = nvt_ipe_proc_cmd_write
};

//=============================================================================
// proc "help" file operation functions
//=============================================================================
static int nvt_ipe_proc_help_show(struct seq_file *sfile, void *v)
{
	seq_printf(sfile, "=====================================================================\n");
...
static int nvt_ipe_proc_help_open(struct inode *inode, struct file *file)
{
	return single_open(file, nvt_ipe_proc_help_show, NULL);
}

static struct file_operations proc_help_fops = {
	.owner  = THIS_MODULE,
	.open   = nvt_ipe_proc_help_open,
	.release = single_release,
	.read   = seq_read,
	.llseek = seq_lseek,
};
