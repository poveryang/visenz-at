...
static DRAM_CONSUME_ATTR nvt_heavyload_cfg2;
static FLGPTN nvt_dram1_heavyload_flag_id;
static FLGPTN nvt_dram2_heavyload_flag_id;
static THREAD_HANDLE nvt_dram1_heavyload_tsk_id;
static THREAD_HANDLE nvt_dram2_heavyload_tsk_id;
static void *handle = NULL;
static void *handle2 = NULL;
#endif
...
static int run_dma_usage(void)
{
	...
	if (p_dma->remain_cnt <= 0) {
		...
		cal_mon_period = (UINT64)p_dma->reset_cnt * dma_get_monitor_period(DDR_ARB_1);
		...
		cal_bandwidth = cal_bandwidth*1000;
		do_div(cal_bandwidth, p_dma->reset_cnt*25*1024*1024);
		p_dma->acc_usage = 0;
...
static int run_dma2_usage(void)
{
	...
	p_dma->remain_cnt--;
	p_dma->acc_usage += dma_get_utilization(DDR_ARB_2);
	p_dma->acc_efficiency += dma_get_efficiency(DDR_ARB_2);
	lock = ddr_arb_platform_spin_lock();
	ddr_cnt[1] +=  dma_get_efficiency(DDR_ARB_2);
	ddr_arb_platform_spin_unlock(lock);

	if (p_dma->remain_cnt <= 0) {
		cal_usage = p_dma->acc_usage*100;
		cal_efficiency = p_dma->acc_efficiency*100;
		cal_mon_period = (UINT64)p_dma->reset_cnt * dma_get_monitor_period(DDR_ARB_2);
		do_div(cal_efficiency, p_dma->acc_usage);
		do_div(cal_usage, cal_mon_period);
		p_dma->usage = (UINT32)cal_usage;
		p_dma->efficiency = (UINT32)cal_efficiency;
...
static THREAD_DECLARE(heavyload_tsk2, arglist)
{
	UINT32 count[1];
	UINT32 test1_start, test2_start, test3_start;
	UINT32 test1_size, test2_size, test3_size;
	...
	BOOL err = FALSE;
	FLGPTN flg_ptn = 0;
	ID flag_id = nvt_dram2_heavyload_flag_id;
	FLGPTN mask = FLGDBGUT_DMA2_HVYLOAD | FLGDBGUT_QUIT_HVY2;

	while (1) {
		wai_flg(&flg_ptn, flag_id, mask, TWF_ORW | TWF_CLR);

		if ((flg_ptn & FLGDBGUT_DMA2_HVYLOAD) != 0U) {
			test1_size =
				test2_size =
					test3_size = ALIGN_FLOOR_32(nvt_heavyload_cfg2.size / 3);

			test1_start = nvt_heavyload_cfg2.addr;
			test2_start = test1_start + test1_size;
			test3_start = test2_start + test2_size;
			...
			DBG_IND("*************ConsumeTsk Information*******************\r\n");
			DBG_IND("*   (@)Loading Degree ==================>[     %05d]*\r\n", nvt_heavyload_cfg2.load_degree);


			// Easy
			if (nvt_heavyload_cfg2.load_degree == DRAM_CONSUME_EASY_LOADING) {
				if (hvy_param0.test_times > 2000) {
					hvy_param0.test_times = 2000;
				}
				DBG_IND("*   (@)Channel0 Burst Length============>[     %05d]*\r\n", hvy_param0.burst_len);
				...
				if (dma_enable_heavyload(DDR_ARB_2, DMA_HEAVY_LOAD_CH0, &hvy_param0) != E_OK) {
					DBG_ERR("Consume task open fail degree[%d]-1\r\n", nvt_heavyload_cfg2.load_degree);
					err = TRUE;
				}

				dma_trig_heavyload(DDR_ARB_2, 0x1);
			}
			// Normal
			else if (nvt_heavyload_cfg2.load_degree == DRAM_CONSUME_NORMAL_LOADING) {
				DBG_IND("*   (@)Channel0 Burst Length============>[     %05d]*\r\n", hvy_param0.burst_len);
				DBG_IND("*   (@)Channel0 Test DMA addr ==========>[0x%08x]*\r\n", hvy_param0.start_addr);
				DBG_IND("*   (@)Channel0 Test DMA size ==========>[0x%08x]*\r\n", hvy_param0.dma_size);
				DBG_IND("*   (@)Channel0 Test count  ============>[     %05d]*\r\n", hvy_param0.test_times);
				...
				DBG_IND("*   (@)Channel1 Test count  ============>[     %05d]*\r\n", hvy_param1.test_times);
				DBG_IND("*************ConsumeTsk Information*******************\r\n");
				if (dma_enable_heavyload(DDR_ARB_2, DMA_HEAVY_LOAD_CH0, &hvy_param1) != E_OK) {
					DBG_ERR("Consume task open fail degree[%d]-1\r\n", nvt_heavyload_cfg2.load_degree);
					err = TRUE;
				}
				if (dma_enable_heavyload(DDR_ARB_2, DMA_HEAVY_LOAD_CH1, &hvy_param1) != E_OK) {
					DBG_ERR("Consume task open fail degree[%d]-2\r\n", nvt_heavyload_cfg2.load_degree);
					err = TRUE;
				}

				dma_trig_heavyload(DDR_ARB_2, 0x3);
			}
			// Heavy
			else {
				DBG_IND("*   (@)Channel0 Burst Length============>[     %05d]*\r\n", hvy_param0.burst_len);
				...
				if (dma_enable_heavyload(DDR_ARB_2, DMA_HEAVY_LOAD_CH2, &hvy_param2) != E_OK) {              // Enable maximun 3 channel at the same time
					DBG_ERR("Consume task open fail degree[%d]-3\r\n", nvt_heavyload_cfg2.load_degree);
					err = TRUE;
				}

				dma_trig_heavyload(DDR_ARB_2, 0x7);
			}

			if (err == FALSE) {
				if (dma_wait_heavyload_done_polling(DDR_ARB_2, NULL) == TRUE) {
					DBG_IND("SUCCESS\r\n");

					if (nvt_heavyload_cfg2.is_start == TRUE)
						set_flg(nvt_dram2_heavyload_flag_id, FLGDBGUT_DMA2_HVYLOAD);
					else
				...
				nvt_heavyload_cfg2.is_start = FALSE;
				fmem_release_from_cma(handle2, 0);
			}
		}
		if ((flg_ptn & FLGDBGUT_QUIT_HVY2) != 0U) {
			break;
		}
	}
	THREAD_RETURN(0);
...
{
	UINT32  pa = 0 , length = 0, id = 0;
	UINT32  enable = 0;
	UINT32  set_mask = 0;

    if(argc<3){
        DBG_DUMP("mwp [ddr_id] [addr] [len] [enable] [set_mask]");
        return -1;
    }

	//sscanf_s(str_cmd, "%x %x", &pa, &length);
	if (argc >= 1) {
		sscanf(pargv[0], "%x", (int *)&id);
	}	
	if (argc >= 2) {
		sscanf(pargv[1], "%x", (int *)&pa);
	}
	if (argc >= 3) {
		sscanf(pargv[2], "%x", (int *)&length);
	}
	if (argc >= 4) {
		sscanf(pargv[3], "%d", (int *)&enable);
	}
	if (argc >= 5) {
		sscanf(pargv[4], "%d", (int *)&set_mask);
	}
    ...
    if(enable)
    {
        ...
        arb_enable_wp(id, WPSET_0, &attrib);
    }
    else {
        arb_disable_wp(id, WPSET_0);
    }
	return 0;
}
...
#if DPROF_PROC_EN
static int nvt_ddr2_proc_monitor_hdl(PMODULE_INFO pmodule_info, unsigned char argc, char **pargv)
{
	int ch, dir;
	...
	printk("argv: %s\r\n", pargv[0]);
	if (strncmp(pargv[0], "start", 5) == 0) {
		dprof_start(DDR_ARB_2, (DPROF_CH)ch, (DMA_DIRECTION)dir);
	} else if (strncmp(pargv[0], "stop", 4) == 0) {
		dprof_dump_and_stop(DDR_ARB_2);
...
static int nvt_dram2_proc_show(struct seq_file *sfile, void *v)
{
		UINT64 cal_usage = 0;
		UINT64 cal_efficiency = 0;
		cal_usage = dma_get_utilization(DDR_ARB_2) * 100;
		do_div(cal_usage, dma_get_monitor_period(DDR_ARB_2));
		cal_efficiency = dma_get_efficiency(DDR_ARB_2) * 100;
		do_div(cal_efficiency, dma_get_utilization(DDR_ARB_2));
...
static int nvt_dram2_heavyload_cfg(PMODULE_INFO pmodule_info, unsigned char argc, char **pargv)
{
	DRAM_CONSUME_ATTR *dram_consume_cfg = &nvt_heavyload_cfg2;
	//struct nvt_fmem_mem_info_t      buf_info = {0};
	//int ret = 0;
	...
	if (argc > 0) {
		sscanf(pargv[0], "%d", (UINT32 *)&dram_consume_cfg->load_degree);
		sscanf(pargv[1], "%x", (UINT32 *)&dram_consume_cfg->addr);
		sscanf(pargv[2], "%d", (UINT32 *)&dram_consume_cfg->size);
	} else {
		nvt_dbg(WRN, "heavyload2 need 3 parameters [degree: 0~2] [addr: 0x...] [size]\r\n");
		return 0;
	}

	//ret = nvt_fmem_mem_info_init(&buf_info, NVT_FMEM_ALLOC_CACHE, dram_consume_cfg->size, NULL);
	//if (ret >= 0) {
	//	handle2 = fmem_alloc_from_cma(&buf_info, 0);
	//} else {
	//	nvt_dbg(WRN, "heavyload2 allocate mem fail!!\r\n");
	//	return ret;
	//}
...
static int nvt_dram2_heavyload_start(PMODULE_INFO pmodule_info, unsigned char argc, char **pargv)
{
	DRAM_CONSUME_ATTR *dram_consume_cfg = &nvt_heavyload_cfg2;

	if (dram_consume_cfg->is_start) {
		nvt_dbg(WRN, "DRAM2 HEAVYLOAD IS START\r\n");
		return 0;
	}
	if (argc > 0) {
		sscanf(pargv[0], "%d", &user_count[1]);
	} else {
		user_count[1] = 0;
	}	
...
static int nvt_dram2_heavyload_stop(PMODULE_INFO pmodule_info, unsigned char argc, char **pargv)
{
	DRAM_CONSUME_ATTR *dram_consume_cfg = &nvt_heavyload_cfg2;

	if (!dram_consume_cfg->is_start) {
		nvt_dbg(WRN, "DRAM2 HEAVYLOAD IS NOT START\r\n");
		return 0;
	}
...
static PROC_CMD nvt_dram2_heavyload_cmd_write_list[] = {
	// keyword   function name
	{ "cfg",     nvt_dram2_heavyload_cfg},
	{ "start",   nvt_dram2_heavyload_start},
	{ "stop",    nvt_dram2_heavyload_stop},
};

#define NVT_DRAM2_HEAVYLOAD_NUM_OF_WRITE_CMD (sizeof(nvt_dram2_heavyload_cmd_write_list) / sizeof(PROC_CMD))

static int nvt_dram2_proc_heavyload_show(struct seq_file *sfile, void *v)
{
    nvt_dbg(IND, "\n");
    return 0;
}

static int nvt_dram2_proc_heavyload_open(struct inode *inode, struct file *file)
{
	return single_open(file, nvt_dram2_proc_heavyload_show, NULL);
}

static ssize_t nvt_dram2_proc_heavyload_cmd_write(struct file *file, const char __user *buf, size_t size, loff_t *off)
{
        int len = size;
        int ret = -EINVAL;
        char cmd_line[MAX_CMD_LENGTH];
	...
	// dispatch command handler
        if (strncmp(argv[0], "r", 2) == 0) {
			...
			for (loop = 0 ; loop < NVT_DRAM2_HEAVYLOAD_NUM_OF_WRITE_CMD ; loop++) {
				if (strncmp(argv[1], nvt_dram2_heavyload_cmd_write_list[loop].cmd, MAX_CMD_LENGTH) == 0) {
				        ret = nvt_dram2_heavyload_cmd_write_list[loop].execute(NULL, ucargc - 2, &argv[2]);
				        break;
				}
			}
	        if (loop >= NVT_DRAM2_HEAVYLOAD_NUM_OF_WRITE_CMD) {
				goto ERR_INVALID_CMD;
	        }
        } else {
            goto ERR_INVALID_CMD;
...
static struct file_operations proc_dram2_heavyload_fops = {
    .owner  = THIS_MODULE,
    .open   = nvt_dram2_proc_heavyload_open,
    .release = single_release,
    .read   = seq_read,
    .llseek = seq_lseek,
    .write   = nvt_dram2_proc_heavyload_cmd_write
};
#endif
