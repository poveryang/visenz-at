...
JPEGENC_RESULT_INFO         g_jpegenc_info;
...
#ifdef __KERNEL__
extern void mjd_mark_engine_end(void);
#endif
...
void jpeg_release_resource(void)
{
	...
	SEM_DESTROY(SEMID_JPEG_Q);

#else
	vos_sem_destroy(SEMID_JPEG);
	vos_sem_destroy(SEMID_JPEG_Q);
	is_create = 0;
#endif
}
...
void jpeg_isr_bottom(UINT32 events)
{
	...
	if (p_trig->cb == NULL) {
		...
		jpg_sts = jpeg_get_status();
		jpg_sts = jpg_sts&JPEG_INT_ALL;

		//printk("callback != NULL\r\n");
		if(p_trig->jpeg_dec_param == NULL)
		{
			p_trig->jpeg_enc_param = (KDRV_VDOENC_PARAM *)g_jpegenc_info.enc_param;
			p_trig->jpeg_enc_param->temproal_id = g_jpegenc_info.job_id;
			if(jpg_sts & JPEG_INT_BUFEND)
			{
				p_trig->jpeg_enc_param->encode_err = 1;
#ifdef __LINUX
				printk("jpg_sts = 0x%x\r\n",jpg_sts);
#endif
			} else {
				p_trig->jpeg_enc_param->encode_err = 0;
				p_trig->jpeg_enc_param->bs_addr_1 = g_jpegenc_info.bs_addr;
				//p_trig->jpeg_enc_param->bs_size_1 = jpeg_get_bssize() + p_trig->jpeg_enc_param->svc_hdr_size;
				p_trig->jpeg_enc_param->bs_size_1 = jpeg_get_bssize() + g_jpegenc_info.bs_hdr_size;
			...
			printk("///ID = 0x%x\r\n", (unsigned int)(p_trig->jpeg_enc_param->temproal_id));
			//printk("jpeg_enc_param->bs_addr_1 = 0x%x\r\n", (unsigned int)(p_trig->jpeg_enc_param->bs_addr_1));
			//printk("p_enc_param->bs_size_1 = 0x%x\r\n", (unsigned int)(p_trig->jpeg_enc_param->bs_size_1));
			//printk("p_trig->user_data = 0x%x\r\n", (unsigned int)(p_trig->jpeg_enc_param->motion_ratio));
			//printk("p_trig->jpeg_enc_param->encode_err  = 0x%x\r\n", (unsigned int)(p_trig->jpeg_enc_param->encode_err ));
#endif
#endif
			jpeg_set_endencode();
			//p_trig->cb->callback(p_trig->jpeg_enc_param, p_trig->user_data);
			//printk("Callback = 0x%x\r\n", (unsigned int)(p_trig->jpeg_enc_param));
			p_trig->cb->callback(p_trig->jpeg_enc_param, (VOID *)g_jpegenc_info.user_data);
		} else {
	...
	if (kdrv_jpeg_queue_is_empty_p(p_trig->p_queue)) {
		...
		{
			//printk("<<<<Next ID = 0x%x\r\n", (unsigned int)(element.jpeg_enc_param.temproal_id));
			p_trig->jpeg_enc_param = element.jpeg_enc_param;
			p_trig->jpeg_enc_param->temproal_id = element.jpeg_enc_param->temproal_id;
			p_trig->jpeg_enc_param->motion_ratio = element.jpeg_enc_param->motion_ratio;
