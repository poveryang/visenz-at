...
typedef struct proc_cmd {
	...
	int (*execute)(PVPE_INFO pdrv, unsigned char argc, char **argv);
} PROC_CMD, *PPROC_CMD;

//============================================================================
// Global variable
//============================================================================
static PVPE_DRV_INFO pdrv_info_data;
...
static PROC_CMD cmd_read_list[] = {
	...
	{ "reg",            nvt_vpe_api_read_reg           },
	{ "dump",            nvt_vpe_api_dump              },
};
...
static PROC_CMD cmd_write_list[] = {
	// keyword          function name
	{ "reg",            nvt_vpe_api_write_reg          },
	{ "pattern",        nvt_vpe_api_write_pattern      },
	{ "vpe_emu",        nvt_vpe_emu                    },
	{ "vpe_kdrv",       nvt_kdrv_vpe_test              },
	{ "vpe_fw_pwsv",    nvt_vpe_fw_power_saving        },
};

#define NUM_OF_WRITE_CMD (sizeof(cmd_write_list) / sizeof(PROC_CMD))

static int nvt_vpe_proc_cmd_show(struct seq_file *sfile, void *v)
{
	//nvt_dbg(IND, "\n");
	return 0;
}

static int nvt_vpe_proc_cmd_open(struct inode *inode, struct file *file)
{
	//nvt_dbg(IND, "\n");
	return single_open(file, nvt_vpe_proc_cmd_show, &pdrv_info_data->vpe_info);
}

static ssize_t nvt_vpe_proc_cmd_write(struct file *file, const char __user *buf,
	...
	if (strncmp(argv[0], "r", 2) == 0) {
		for (loop = 0 ; loop < NUM_OF_READ_CMD; loop++) {
			if (strncmp(argv[1], cmd_read_list[loop].cmd, MAX_CMD_LENGTH) == 0) {
				ret = cmd_read_list[loop].execute(&pdrv_info_data->vpe_info, ucargc - 2, &argv[2]);
				break;
			}
		}
		...
		for (loop = 0 ; loop < NUM_OF_WRITE_CMD ; loop++) {
			if (strncmp(argv[1], cmd_write_list[loop].cmd, MAX_CMD_LENGTH) == 0) {
				ret = cmd_write_list[loop].execute(&pdrv_info_data->vpe_info, ucargc - 2, &argv[2]);
				break;
			}
		}
...
static struct file_operations proc_cmd_fops = {
	.owner   = THIS_MODULE,
	.open    = nvt_vpe_proc_cmd_open,
	.read    = seq_read,
	.llseek  = seq_lseek,
	.release = single_release,
	.write   = nvt_vpe_proc_cmd_write
};

//=============================================================================
// proc "help" file operation functions
//=============================================================================
static int nvt_vpe_proc_help_show(struct seq_file *sfile, void *v)
{
	seq_printf(sfile, "=====================================================================\n");
...
static int nvt_vpe_proc_help_open(struct inode *inode, struct file *file)
{
	return single_open(file, nvt_vpe_proc_help_show, NULL);
}

static struct file_operations proc_help_fops = {
	.owner  = THIS_MODULE,
	.open   = nvt_vpe_proc_help_open,
	.release = single_release,
	.read   = seq_read,
	.llseek = seq_lseek,
};
...
int nvt_vpe_proc_init(PVPE_DRV_INFO pdrv_info)
{
	...
	//printk("nvt_vpe_proc_init...\r\n");

	pmodule_root = proc_mkdir("kdrv_vpe", NULL);
	if (pmodule_root == NULL) {
		nvt_dbg(ERR, "failed to create Module root\n");
		ret = -EINVAL;
		goto remove_root;
	}
	pdrv_info->pproc_vpe_root = pmodule_root;
...
remove_root:
	proc_remove(pdrv_info->pproc_vpe_root);
	return ret;
}
