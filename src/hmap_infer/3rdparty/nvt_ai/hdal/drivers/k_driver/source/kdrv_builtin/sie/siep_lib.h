...
#define SIE_INT_SIECLK_ERR         0x00004000 ///< Enable interrupt
...
#define SIE_INT_BEHAVIOR_ERR       0x00040000 ///< Enable interrupt
...
typedef enum {
	...
	SIE_CHG_GRIDLINE,               ///< refer to "SIE_GRIDLINE_INFO"       , change Grid Line
...
typedef struct _SIE_BURST_LENGTH {
	...
	SIE_BURST_SEL       BurstLenOut3; ///< for SIE1/2/3/4
	SIE_BURST_SEL       BurstLenOut4; ///< for SIE1
	SIE_BURST_SEL       BurstLenOut5; ///< for SIE1/2/3/4
...
typedef struct {
    UINT32              uiBrcSegBitNo[SIE_ENGINE_ID_MAX];      ///< Raw encoder - BRC target encoded bit number per segment
	BOOL                bBccGammaEn;
...
typedef struct _SIE_OPENOBJ {
	void (*pfSieIsrCb)(UINT32 uiIntpStatus, SIE_ENGINE_STATUS_INFO_CB *info); ///< isr callback function
