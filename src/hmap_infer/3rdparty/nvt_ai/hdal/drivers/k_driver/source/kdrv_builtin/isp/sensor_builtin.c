...
static SENSOR_BUILTIN_DFT_PARAM imx290_linear_dft_param = {
	.pclk =      148500000,
	.hd =        4400,
	.vd =        1125,
	.fps =       3000,
...
static SENSOR_BUILTIN_DFT_PARAM imx290_shdr_dft_param = {
	.pclk =      148500000,
	.hd =        1100,
	.vd =        2250,
	.fps =       3000,
...
static SENSOR_BUILTIN_DFT_PARAM f37_linear_dft_param = {
	.pclk =      129600000,
	.hd =        3840,
	.vd =        1125,
	.fps =       3000,
...
static SENSOR_BUILTIN_DFT_PARAM os02k10_linear_dft_param = {
	// NOTE: mode 4
	.pclk =      108000000,
	.hd =        1440,
	.vd =        1250,
...
static SENSOR_BUILTIN_DFT_PARAM os02k10_shdr_dft_param = {
	// NOTE: mode 2
	.pclk =      108000000,
	.hd =        1224,
	.vd =        1470,
...
static SENSOR_BUILTIN_DFT_PARAM os05a10_linear_dft_param = {
	.pclk = 108000000,
	.hd = 1540,
	.vd = 2337,
	.fps = 3000,
...
static SENSOR_BUILTIN_DFT_PARAM os05a10_shdr_dft_param = {
	.pclk = 54000000,
	.hd = 720,
	.vd = 2496,
	.fps = 3000,
...
static SENSOR_BUILTIN_DFT_PARAM f35_linear_dft_param = {
	.pclk =      40500000,
	.hd =        1200,
	.vd =        1125,
	.fps =       3000,
...
static SENSOR_BUILTIN_DFT_PARAM f35_shdr_dft_param = {
	.pclk =      81000000,
	.hd =        600,
	.vd =        2250,
	.fps =       3000,
...
static SENSOR_BUILTIN_DFT_PARAM gc4653_linear_dft_param = {
	.pclk =      216000000,
	.hd =        4800,
	.vd =        1500,
	.fps =       3000,
...
static UINT32 sensor_builtin_imx290_get_row_time(UINT32 id)
{
	return (100 * imx290_linear_dft_param.hd / (imx290_linear_dft_param.pclk / 100000));
}

static UINT32 sensor_builtin_imx290_get_min_expt_time(UINT32 id)
{
	return (sensor_builtin_imx290_get_row_time(id) * imx290_linear_dft_param.min_expt_line / 10 + 1);
}

static void sensor_builtin_imx290_print_info(UINT32 id)
{
	printk("imx290: pclk %d, hd %d, vd %d, fps %d, frame_num %d, max gin %d, min expt line %d, row time %d \r\n", 
																							imx290_linear_dft_param.pclk, 
																							imx290_linear_dft_param.hd, 
																							imx290_linear_dft_param.vd, 
																							imx290_linear_dft_param.fps, 
																							imx290_linear_dft_param.frame_num, 
																							imx290_linear_dft_param.max_gain, 
																							imx290_linear_dft_param.min_expt_line, 
																							sensor_builtin_imx290_get_min_expt_time(id));
}
...
static void sensor_builtin_imx290_set_gain(UINT32 id, ISP_BUILTIN_SENSOR_CTRL *sensor_ctrl)
{
	...
	if (isp_builtin_get_shdr_enable(id)) {
		total_frame = imx290_shdr_dft_param.frame_num;
	} else {
		total_frame = imx290_linear_dft_param.frame_num;
	}
...
static void sensor_builtin_imx290_set_expt(UINT32 id, ISP_BUILTIN_SENSOR_CTRL *sensor_ctrl)
{
	...
	UINT32 sensor_dft_vd = 1125, dft_fps = 3000;
	static UINT32 pre_short_expt = 0xFFFFFFFF;

	if (isp_builtin_get_shdr_enable(id)) {
		sensor_dft_vd = imx290_shdr_dft_param.vd;
		dft_fps =       imx290_shdr_dft_param.fps;
		total_frame =   imx290_shdr_dft_param.frame_num;
	} else {
		sensor_dft_vd = imx290_linear_dft_param.vd;
		dft_fps =       imx290_linear_dft_param.fps;
		total_frame =   imx290_linear_dft_param.frame_num;
	}
	...
	for (frame_cnt = 0; frame_cnt < total_frame; frame_cnt++) {
		t_row = sensor_builtin_imx290_get_row_time(id);
...
static UINT32 sensor_builtin_f37_get_row_time(UINT32 id)
{
	return (100 * f37_linear_dft_param.hd / (f37_linear_dft_param.pclk / 100000));
}

static UINT32 sensor_builtin_f37_get_min_expt_time(UINT32 id)
{
	return (sensor_builtin_f37_get_row_time(id) * f37_linear_dft_param.min_expt_line / 10 + 1);
}

static void sensor_builtin_f37_print_info(UINT32 id)
{
	printk("f37: pclk %d, hd %d, vd %d, fps %d, frame_num %d, max gin %d, min expt line %d, row time %d \r\n", 
																							f37_linear_dft_param.pclk, 
																							f37_linear_dft_param.hd, 
																							f37_linear_dft_param.vd, 
																							f37_linear_dft_param.fps, 
																							f37_linear_dft_param.frame_num, 
																							f37_linear_dft_param.max_gain, 
																							f37_linear_dft_param.min_expt_line, 
																							sensor_builtin_f37_get_min_expt_time(id));
}
...
static void sensor_builtin_f37_set_expt(UINT32 id, ISP_BUILTIN_SENSOR_CTRL *sensor_ctrl)
{
	...
	sensor_dft_vd = f37_linear_dft_param.vd;
	dft_fps = f37_linear_dft_param.fps;

	exp_time = sensor_ctrl->exp_time[0];

	// Calculate exposure time
	t_row = sensor_builtin_f37_get_row_time(id);
...
static UINT32 sensor_builtin_os02k10_get_row_time(UINT32 id)
{
	if (isp_builtin_get_shdr_enable(id)) {
		return (100 * os02k10_shdr_dft_param.hd / (os02k10_shdr_dft_param.pclk / 2 / 100000));
	} else {
		return (100 * os02k10_linear_dft_param.hd / (os02k10_linear_dft_param.pclk / 100000));
	}
}

static UINT32 sensor_builtin_os02k10_get_min_expt_time(UINT32 id)
{
	return (sensor_builtin_os02k10_get_row_time(id) * os02k10_linear_dft_param.min_expt_line / 10 + 1);
}

static void sensor_builtin_os02k10_print_info(UINT32 id)
{
	printk("os02k10: pclk %d, hd %d, vd %d, fps %d, frame_num %d, max gin %d, min expt line %d, row time %d \r\n", 
																							os02k10_linear_dft_param.pclk, 
																							os02k10_linear_dft_param.hd, 
																							os02k10_linear_dft_param.vd, 
																							os02k10_linear_dft_param.fps, 
																							os02k10_linear_dft_param.frame_num, 
																							os02k10_linear_dft_param.max_gain, 
																							os02k10_linear_dft_param.min_expt_line, 
																							sensor_builtin_os02k10_get_min_expt_time(id));
}
...
static void sensor_builtin_os02k10_set_gain(UINT32 id, ISP_BUILTIN_SENSOR_CTRL *sensor_ctrl)
{
	...
	if (isp_builtin_get_shdr_enable(id)) {
		total_frame = os02k10_shdr_dft_param.frame_num;
	} else {
		total_frame = os02k10_linear_dft_param.frame_num;
	}
	...
	for (frame_cnt = 0; frame_cnt < total_frame; frame_cnt++) {
		...
		} else if (gain_info.gain_ratio[frame_cnt] > os02k10_linear_dft_param.max_gain) {
			gain_info.gain_ratio[frame_cnt] = os02k10_linear_dft_param.max_gain;
		}
...
static void sensor_builtin_os02k10_set_expt(UINT32 id, ISP_BUILTIN_SENSOR_CTRL *sensor_ctrl)
{
	...
	UINT32 sensor_dft_vd = 1250, dft_fps = 3000;

	if (isp_builtin_get_shdr_enable(id)) {
		sensor_dft_vd = os02k10_shdr_dft_param.vd;
		dft_fps =       os02k10_shdr_dft_param.fps;
		total_frame =   os02k10_shdr_dft_param.frame_num;
	} else {
		sensor_dft_vd = os02k10_linear_dft_param.vd;
		dft_fps =       os02k10_linear_dft_param.fps;
		total_frame =   os02k10_linear_dft_param.frame_num;
	}
	...
	for (frame_cnt = 0; frame_cnt < total_frame; frame_cnt++) {
		t_row = sensor_builtin_os02k10_get_row_time(id);
...
static UINT32 sensor_builtin_os05a10_get_row_time(UINT32 id)
{
	if (isp_builtin_get_shdr_enable(id)) {
		return (100 * os05a10_shdr_dft_param.hd / (os05a10_shdr_dft_param.pclk / 2 / 100000));
	} else {
		return (100 * os05a10_linear_dft_param.hd / (os05a10_linear_dft_param.pclk / 100000));
	}
}

static UINT32 sensor_builtin_os05a10_get_min_expt_time(UINT32 id)
{
	return (sensor_builtin_os05a10_get_row_time(id) * os05a10_linear_dft_param.min_expt_line / 10 + 1);
}

static void sensor_builtin_os05a10_print_info(UINT32 id)
{
	printk("os05a10: pclk %d, hd %d, vd %d, fps %d, frame_num %d, max gin %d, min expt line %d, row time %d \r\n", 
																							os05a10_linear_dft_param.pclk, 
																							os05a10_linear_dft_param.hd, 
																							os05a10_linear_dft_param.vd, 
																							os05a10_linear_dft_param.fps, 
																							os05a10_linear_dft_param.frame_num, 
																							os05a10_linear_dft_param.max_gain, 
																							os05a10_linear_dft_param.min_expt_line, 
																							sensor_builtin_os05a10_get_min_expt_time(id));
}
...
static void sensor_builtin_os05a10_set_gain(UINT32 id, ISP_BUILTIN_SENSOR_CTRL *sensor_ctrl)
{
	...
	// Calculate sensor gain
	if (isp_builtin_get_shdr_enable(id)) {
		total_frame = os05a10_shdr_dft_param.frame_num;
	} else {
		total_frame = os05a10_linear_dft_param.frame_num;
	}
	...
	for (frame_cnt = 0; frame_cnt < total_frame; frame_cnt++) {
		...
		if ( 15500 >= gain_info.gain_ratio[frame_cnt] ) {
			...
			awb_gain = 0x400;
		} else if (248000 >= gain_info.gain_ratio[frame_cnt]) {
			d_gain=(gain_info.gain_ratio[frame_cnt]) * 1024 / 15500;
			a_gain= 0x7C0;
			awb_gain = 0x400;
		} else {
			d_gain= 0x3FFF;
			a_gain= 0x07C0;
			awb_gain = (gain_info.gain_ratio[frame_cnt]) * 1024 / 248000;
		}
	...
	if (!isp_builtin_get_shdr_enable(id)) {
		...
		//Set awb gain
		cmd = sensor_builtin_set_cmd_info(0x5100, 1, ((data4[0]) >> 8) & 0x0F, 0x0);
		sensor_builtin_write_i2c_2b1b(id, &cmd);
		cmd = sensor_builtin_set_cmd_info(0x5101, 1, (data4[0])& 0xFF, 0x0);
		sensor_builtin_write_i2c_2b1b(id, &cmd);

		cmd = sensor_builtin_set_cmd_info(0x5102, 1, ((data4[0]) >> 8) & 0x0F, 0x0);
		sensor_builtin_write_i2c_2b1b(id, &cmd);
		cmd = sensor_builtin_set_cmd_info(0x5103, 1, (data4[0])& 0xFF, 0x0);
		sensor_builtin_write_i2c_2b1b(id, &cmd);

		cmd = sensor_builtin_set_cmd_info(0x5104, 1, ((data4[0]) >> 8) & 0x0F, 0x0);
		sensor_builtin_write_i2c_2b1b(id, &cmd);
		cmd = sensor_builtin_set_cmd_info(0x5105, 1, (data4[0])& 0xFF, 0x0);
		sensor_builtin_write_i2c_2b1b(id, &cmd);
	} else {
		...
		cmd = sensor_builtin_set_cmd_info(0x5140, 1, ((data4[1]) >> 8) & 0x0F, 0x0);
		sensor_builtin_write_i2c_2b1b(id, &cmd);
		cmd = sensor_builtin_set_cmd_info(0x5141, 1, (data4[1])& 0xFF, 0x0);
		sensor_builtin_write_i2c_2b1b(id, &cmd);

		cmd = sensor_builtin_set_cmd_info(0x5142, 1, ((data4[1]) >> 8) & 0x0F, 0x0);
		sensor_builtin_write_i2c_2b1b(id, &cmd);
		cmd = sensor_builtin_set_cmd_info(0x5143, 1, (data4[1])& 0xFF, 0x0);
		sensor_builtin_write_i2c_2b1b(id, &cmd);

		cmd = sensor_builtin_set_cmd_info(0x5144, 1, ((data4[1]) >> 8) & 0x0F, 0x0);
		sensor_builtin_write_i2c_2b1b(id, &cmd);
		cmd = sensor_builtin_set_cmd_info(0x5145, 1, (data4[1])& 0xFF, 0x0);
		sensor_builtin_write_i2c_2b1b(id, &cmd);
	}
	// Set BLC trigger
...
static void sensor_builtin_os05a10_set_expt(UINT32 id, ISP_BUILTIN_SENSOR_CTRL *sensor_ctrl)
{
	...
	// Calculate exposure line
	for (frame_cnt = 0; frame_cnt < total_frame; frame_cnt++) {
		t_row = sensor_builtin_os05a10_get_row_time(id);
...
static UINT32 sensor_builtin_f35_get_row_time(UINT32 id)
{
	if (isp_builtin_get_shdr_enable(id)) {
		return (100 * f35_shdr_dft_param.hd / (f35_shdr_dft_param.pclk / 2 / 100000));
	} else {
		return (100 * f35_linear_dft_param.hd / (f35_linear_dft_param.pclk / 100000));
	}
}

static UINT32 sensor_builtin_f35_get_min_expt_time(UINT32 id)
{
	return (sensor_builtin_f35_get_row_time(id) * f35_linear_dft_param.min_expt_line / 10 + 1);
}

static void sensor_builtin_f35_print_info(UINT32 id)
{
	printk("f35: pclk %d, hd %d, vd %d, fps %d, frame_num %d, max gin %d, min expt line %d, row time %d \r\n", 
																							f35_linear_dft_param.pclk, 
																							f35_linear_dft_param.hd, 
																							f35_linear_dft_param.vd, 
																							f35_linear_dft_param.fps, 
																							f35_linear_dft_param.frame_num, 
																							f35_linear_dft_param.max_gain, 
																							f35_linear_dft_param.min_expt_line, 
																							sensor_builtin_f35_get_min_expt_time(id));
}
...
static void sensor_builtin_f35_set_gain(UINT32 id, ISP_BUILTIN_SENSOR_CTRL *sensor_ctrl)
{
	...
	// Calculate sensor gain
	if (isp_builtin_get_shdr_enable(id)) {
		total_frame = f35_shdr_dft_param.frame_num;
	} else {
		total_frame = f35_linear_dft_param.frame_num;
	}

	for (frame_cnt = 0; frame_cnt < total_frame; frame_cnt++) {
		if (gain_info.gain_ratio[frame_cnt] < 1000) {
			gain_info.gain_ratio[frame_cnt] = 1000;
		} else if (gain_info.gain_ratio[frame_cnt] > f35_linear_dft_param.max_gain) {
			gain_info.gain_ratio[frame_cnt] = f35_linear_dft_param.max_gain;
		}
...
static void sensor_builtin_f35_set_expt(UINT32 id, ISP_BUILTIN_SENSOR_CTRL *sensor_ctrl)
{
	...
	if (isp_builtin_get_shdr_enable(id)) {
		sensor_dft_vd = f35_shdr_dft_param.vd;
		dft_fps =       f35_shdr_dft_param.fps;
		total_frame =   f35_shdr_dft_param.frame_num;
	} else {
		sensor_dft_vd = f35_linear_dft_param.vd;
		dft_fps =       f35_linear_dft_param.fps;
		total_frame =   f35_linear_dft_param.frame_num;
	}

	// Calculate exposure line
	for (frame_cnt = 0; frame_cnt < total_frame; frame_cnt++) {
		t_row = sensor_builtin_f35_get_row_time(id);
...
static UINT32 sensor_builtin_gc4653_get_row_time(UINT32 id)
{
	return (100 * gc4653_linear_dft_param.hd / (gc4653_linear_dft_param.pclk / 100000));
}

static UINT32 sensor_builtin_gc4653_get_min_expt_time(UINT32 id)
{
	return (sensor_builtin_gc4653_get_row_time(id) * gc4653_linear_dft_param.min_expt_line / 10 + 1);
}

static void sensor_builtin_gc4653_print_info(UINT32 id)
{
	printk("gc4653: pclk %d, hd %d, vd %d, fps %d, frame_num %d, max gin %d, min expt line %d, row time %d \r\n", 
																							gc4653_linear_dft_param.pclk, 
																							gc4653_linear_dft_param.hd, 
																							gc4653_linear_dft_param.vd, 
																							gc4653_linear_dft_param.fps, 
																							gc4653_linear_dft_param.frame_num, 
																							gc4653_linear_dft_param.max_gain, 
																							gc4653_linear_dft_param.min_expt_line, 
																							sensor_builtin_gc4653_get_min_expt_time(id));
}
...
static void sensor_builtin_gc4653_set_gain(UINT32 id, ISP_BUILTIN_SENSOR_CTRL *sensor_ctrl)
{
	...
	} else if (gain_info.gain_ratio[0] > gc4653_linear_dft_param.max_gain) {
		gain_info.gain_ratio[0] = gc4653_linear_dft_param.max_gain;
	}
...
static void sensor_builtin_gc4653_set_expt(UINT32 id, ISP_BUILTIN_SENSOR_CTRL *sensor_ctrl)
{
	...
	SENSOR_BUILTIN_I2C_CMD cmd;
	UINT32 t_row = 0, sensor_vd;
	UINT32 sensor_dft_vd = 1500, dft_fps = 3000;

	sensor_dft_vd = gc4653_linear_dft_param.vd;
	dft_fps =       gc4653_linear_dft_param.fps;

	// calculate exposure line
	t_row = sensor_builtin_gc4653_get_row_time(id);
	if (t_row == 0) {
...
UINT32 sensor_builtin_get_row_time(UINT32 id)
{
	switch (isp_builtin_get_sensor_name(id)) {
		default:
		case 0:
			return sensor_builtin_imx290_get_row_time(id);
		break;
		case 1:
			return sensor_builtin_f37_get_row_time(id);
		break;
		case 2:
			return sensor_builtin_os02k10_get_row_time(id);
		break;
		case 3:
			return sensor_builtin_os05a10_get_row_time(id);
		break;
		case 4:
			return sensor_builtin_f35_get_row_time(id);
		break;
		case 5:
			return sensor_builtin_gc4653_get_row_time(id);
		break;
	}
}
...
UINT32 sensor_builtin_get_min_expt_time(UINT32 id)
{
	switch (isp_builtin_get_sensor_name(id)) {
		default:
		case 0:
			return sensor_builtin_imx290_get_min_expt_time(id);
		break;
		case 1:
			return sensor_builtin_f37_get_min_expt_time(id);
		break;
		case 2:
			return sensor_builtin_os02k10_get_min_expt_time(id);
		break;
		case 3:
			return sensor_builtin_os05a10_get_min_expt_time(id);
		break;
		case 4:
			return sensor_builtin_f35_get_min_expt_time(id);
		break;
		case 5:
			return sensor_builtin_gc4653_get_min_expt_time(id);
		break;
	}
}
...
void sensor_builtin_print_info(UINT32 id)
{
	switch (isp_builtin_get_sensor_name(id)) {
		default:
		case 0:
			return sensor_builtin_imx290_print_info(id);
		break;
		case 1:
			return sensor_builtin_f37_print_info(id);
		break;
		case 2:
			return sensor_builtin_os02k10_print_info(id);
		break;
		case 3:
			return sensor_builtin_os05a10_print_info(id);
		break;
		case 4:
			return sensor_builtin_f35_print_info(id);
		break;
		case 5:
			return sensor_builtin_gc4653_print_info(id);
		break;
	}
}
