...
typedef struct proc_cmd {
	...
	int (*execute)(PDCE_INFO pdrv, unsigned char argc, char **argv);
...
// Global variable
//============================================================================
static PDCE_DRV_INFO pdrv_info_data;
...
static PROC_CMD cmd_read_list[] = {
	...
	{ "reg",            nvt_dce_api_read_reg,             "Read DCE registers."},
	{ "dump",           nvt_dce_api_dump,                 "Dump Kdrv parameters."},
	{ "show_stripe",	nvt_dce_show_stripe_settings,"Show DCE stripe settings."},
#if !defined (CONFIG_NVT_SMALL_HDAL)
	{ "api_log",        nvt_dce_api_control_api_log,      "Dump DCE control operation log."},
#endif
};
...
static PROC_CMD cmd_write_list[] = {
	// keyword          function name                     description
	{ "reg",            nvt_dce_api_write_reg,            "Write DCE registers."},
	//{ "pattern",        nvt_dce_api_write_pattern,        "N/A."},
#if !defined (CONFIG_NVT_SMALL_HDAL)
	{ "kdrvtest",       nvt_dce_api_test_kdrv_run_d2d,    "DCE D2D process."},
	{ "hw_pwsv",        nvt_dce_api_hw_power_saving,      "Enable/Disable HW power saving."},
	{ "fw_pwsv",        nvt_dce_api_fw_power_saving,      "Enable/Disable FW power saving."},
#endif
};

#define NUM_OF_WRITE_CMD (sizeof(cmd_write_list) / sizeof(PROC_CMD))

static int nvt_dce_proc_cmd_show(struct seq_file *sfile, void *v)
{
	DBG_MSG("\n");
	return 0;
}

static int nvt_dce_proc_cmd_open(struct inode *inode, struct file *file)
{
	DBG_MSG("\n");
	return single_open(file, nvt_dce_proc_cmd_show, &pdrv_info_data->module_info);
}

static ssize_t nvt_dce_proc_cmd_write(struct file *file, const char __user *buf,
...
static struct file_operations proc_cmd_fops = {
	.owner  = THIS_MODULE,
	.open   = nvt_dce_proc_cmd_open,
	.read   = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
	.write  = nvt_dce_proc_cmd_write
};

//=============================================================================
// proc "help" file operation functions
//=============================================================================
static int nvt_dce_proc_help_show(struct seq_file *sfile, void *v)
{
	int loop;

	seq_printf(sfile, "=====================================================================\n");
	seq_printf(sfile, " DCE command list\n");
	seq_printf(sfile, "=====================================================================\n");
...
static int nvt_dce_proc_help_open(struct inode *inode, struct file *file)
{
	return single_open(file, nvt_dce_proc_help_show, NULL);
}

static struct file_operations proc_help_fops = {
	.owner  = THIS_MODULE,
	.open   = nvt_dce_proc_help_open,
	.release = single_release,
	.read   = seq_read,
	.llseek = seq_lseek,
};

int nvt_dce_proc_init(PDCE_DRV_INFO pdrv_info)
{
	int ret = 0;
...
int nvt_dce_proc_remove(PDCE_DRV_INFO pdrv_info)
{
