...
typedef struct proc_cmd {
	...
	int (*execute)(PIFE_MODULE_INFO pdrv, unsigned char argc, char **argv);
} PROC_CMD, *PPROC_CMD;

//============================================================================
// Global variable
//============================================================================
static PIFE_DRV_INFO pdrv_info_data;
...
static PROC_CMD cmd_read_list[] = {
	...
	{ "reg",            nvt_ife_api_read_reg           }, // read reg
	{ "dump",           nvt_ife_api_dump               }  // dump debug
};
...
static PROC_CMD cmd_write_list[] = {
	// keyword          function name
	{ "reg",            nvt_ife_api_write_reg           },// write value to hw
#if !defined (CONFIG_NVT_SMALL_HDAL)	
	{ "pattern",        nvt_kdrv_ipp_api_ife_test       }, // read file from sd:
	{ "hw_pwsv",        nvt_ife_api_hw_power_saving,    }, //"Enable/Disable HW power saving."
	{ "fw_pwsv",        nvt_ife_api_fw_power_saving,    }  //"Enable/Disable FW power saving."
#endif	
};

#define NUM_OF_WRITE_CMD (sizeof(cmd_write_list) / sizeof(PROC_CMD))

static int nvt_ife_proc_cmd_show(struct seq_file *sfile, void *v)
{
	nvt_dbg(IND, "\n");
	return 0;
}

static int nvt_ife_proc_cmd_open(struct inode *inode, struct file *file)
{
	nvt_dbg(IND, "\n");
	return single_open(file, nvt_ife_proc_cmd_show, &pdrv_info_data->module_info);
}

static ssize_t nvt_ife_proc_cmd_write(struct file *file, const char __user *buf,
...
static struct file_operations proc_cmd_fops = {
	.owner   = THIS_MODULE,
	.open    = nvt_ife_proc_cmd_open,
	.read    = seq_read,
	.llseek  = seq_lseek,
	.release = single_release,
	.write   = nvt_ife_proc_cmd_write
};

//=============================================================================
// proc "help" file operation functions
//=============================================================================
static int nvt_ife_proc_help_show(struct seq_file *sfile, void *v)
{
	seq_printf(sfile, "=====================================================================\n");
...
static int nvt_ife_proc_help_open(struct inode *inode, struct file *file)
{
	return single_open(file, nvt_ife_proc_help_show, NULL);
}

static struct file_operations proc_help_fops = {
	.owner  = THIS_MODULE,
	.open   = nvt_ife_proc_help_open,
	.release = single_release,
	.read   = seq_read,
	.llseek = seq_lseek,
};

int nvt_ife_proc_init(PIFE_DRV_INFO pdrv_info)
{
	int ret = 0;
...
int nvt_ife_proc_remove(PIFE_DRV_INFO pdrv_info)
{
