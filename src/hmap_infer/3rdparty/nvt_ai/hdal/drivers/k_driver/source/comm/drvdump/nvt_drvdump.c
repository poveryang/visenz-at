...
static int nvt_drvdump_proc_summary_show(struct seq_file *sfile, void *v)
{
	...
	sdio_drvdump();
	pr_info("####### CSI ######\n");
	csi_print_info_to_uart(NULL);
	csi2_print_info_to_uart(NULL);
	return 0;
}
...
extern BOOL csi_print_info_to_uart(CHAR *pstrcmd);;
extern BOOL csi2_print_info_to_uart(CHAR *pstrcmd);
/**
