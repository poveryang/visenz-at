...
static DRAM_CONSUME_ATTR nvt_heavyload_cfg2 = {0};
static ID nvt_dram1_heavyload_flag_id;
static ID nvt_dram2_heavyload_flag_id;
...
void dram2_consume_task(void)
{
	...
	while (1) {
		...
		if ((flg_ptn & FLGDBGUT_DMA2_HVYLOAD) != 0U) {
			if (nvt_heavyload_cfg2.load_degree == DRAM_CONSUME_CH_DISABLE) {
				// Scan which DMA channel is required to do disable/enable test
				for (dma_grp = 0; dma_grp < DMA_CH_GROUP_CNT; dma_grp++) {
					ch_bit = nvt_heavyload_cfg2.dma_channel.channel_group[dma_grp];
				...
				DBG_DUMP("*************ConsumeTsk Information*******************\r\n");
				DBG_DUMP("*   (@)Loading Degree ==================>[     %05d]*\r\n", (int)nvt_heavyload_cfg2.load_degree);
...
int dram2_consume_cfg(PDRAM_CONSUME_ATTR attr)
{
...
irqreturn_t ddr_platform_isr2(int irq, void *devid)
{
	arb2_isr(0);
	return IRQ_HANDLED;
}
void ddr_arb_platform_create_resource(void)
{
	if (!is_create) {
		...
		THREAD_CREATE(nvt_dram2_heavyload_tsk_id, dram2_consume_task, NULL, "dram2_consume_task");
		THREAD_RESUME(nvt_dram2_heavyload_tsk_id);

		request_irq(INT_ID_DMA, ddr_platform_isr, IRQF_TRIGGER_HIGH, "dma", 0);
		request_irq(INT_ID_DMA2, ddr_platform_isr2, IRQF_TRIGGER_HIGH, "dma2", 0);
...
void ddr_arb_platform_create_resource(MODULE_INFO *pmodule_info)
{
	...
	IOADDR_DDR_ARB2_REG_BASE = (UINT32)pmodule_info->io_addr[1];
	IOADDR_DDR_ARB_PROT_REG_BASE = (UINT32)pmodule_info->io_addr[2];
	IOADDR_DDR_ARB2_PROT_REG_BASE = (UINT32)pmodule_info->io_addr[3];
