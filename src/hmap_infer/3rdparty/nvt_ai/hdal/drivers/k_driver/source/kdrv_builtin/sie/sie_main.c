...
UINT32 sie_power_saving_clkrate = FALSE;
...
static int nvt_sie_probe(struct platform_device * pdev)
{
	...
	if (of_property_read_u32(pdev->dev.of_node, "power_saving", &sie_power_saving_mode) == 0) {
	}else {
		nvt_dbg(ERR, "parse sie power_saving failed\r\n");
	}

	if (of_property_read_u32(pdev->dev.of_node, "power_saving_clkrate", &sie_power_saving_clkrate) == 0) {
	}else {
		nvt_dbg(ERR, "parse sie power_saving_clkrate failed\r\n");
	}
