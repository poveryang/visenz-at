...
AI_IOC_UNINIT_LINK_INFO_EXIT:
	...
	case NUE_IOC_RUN_FC: {
		...
		input_pa = nue_parm.dmaio_addr.addrin0;
		nue_parm.dmaio_addr.addrin0 = kdrv_ai_pa2va_remap_wo_sync(nue_parm.dmaio_addr.addrin0, ioremap_size);
		// output
		...
		for (run_idx = 0; run_idx < run_time; run_idx++) {
			...
			nue_parm.svm_parm.fcd_parm.fcd_enc_bit_length = input_parm.fc_ker.weight_w*nue_parm.insvm_size.sv_h*8;
...
NUE_IOC_RUN_FC_EXIT:
		...
		// unmap va
		// input
		kdrv_ai_pa2va_unmap(nue_parm.dmaio_addr.addrin0, input_pa);
		// output
...
#if (KDRV_AI_MINI_FOR_FASTBOOT == 2 || KDRV_AI_MINI_FOR_FASTBOOT == 1)
        nvt_dbg(ERR, "AI_FASTBOOT: Error, NUE2_IOC_RUN2 can't be called at fastboot.\n");
#else
