...
static struct proc_dir_entry *hdal_proc, *hdal_setting_proc, *hdal_flow_proc, *hdal_version_proc, *hdal_err_proc;
...
void __exit log_clearnup(void)
{
	...
	if (modeproc != 0) {
		remove_proc_entry("log_mode", hdal_proc);
	}
	if (dump2proc != 0) {
		remove_proc_entry("log_dump", hdal_proc);
	}
