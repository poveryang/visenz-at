...
struct ISO29158Quality
{
    ...
    ISO29158Quality()
        : overall(-1), decode(-1), cc(-1), cm(-1), r_target(-1), an(-1), gn(-1),
          uec(-1), ddg(-1), cc_value(-1), cm_value(-1), r_target_value(-1),
          an_value(-1), gn_value(-1), uec_value(-1), ddg_value(-1)
    {
    }
};
...
struct BarcodeInfo
{
    ...
    float unit_width_ratio; // width ratio of unit bright and dark module for
                            // two width
    std::string bit_str;    // onde code bit string
