...
{
    std::string::size_type idx = folder_path.rfind('.');
    if (idx != std::string::npos)
    {
        // 获取扩展名并转换为小写
        std::string extension = cv::toLowerCase(folder_path.substr(idx + 1));
        // 判断扩展名是否为常见图片格式
        if (extension == "jpg" || extension == "jpeg" || extension == "png" ||
            extension == "bmp" || extension == "gif" || extension == "tiff" ||
            extension == "webp")
        {
            input_image_list.emplace_back(folder_path);
            return;
        }
    }
...
smartmore::barcode::BarcodeType GetBarcodeDLType(int type)
{
    ...
    {
        return smartmore::barcode::BarcodeType::kMicroQR;
    }
    else
    {
                         ...
                         double time, bool status)
{
    ...
    for (int i = 0; i < output.results.size(); i++)
    {
        ...
        else if (info.type == smartmore::barcode::BarcodeType::kVericode)
        {
            result["type"] = "vericode";
        }
        ...
        else if (info.type == smartmore::barcode::BarcodeType::kMicroQR)
        {
            result["type"] = "mqr";
        }
                         ...
                         const smartmore::barcode::BarcodeType &code_type)
{
    ...
    JudgeCodeType(smartmore::barcode::BarcodeType::kPdf417, "PDF417", code_type,
                  output);
    JudgeCodeType(smartmore::barcode::BarcodeType::kVericode, "Vericode",
                  code_type, output);
    JudgeCodeType(smartmore::barcode::BarcodeType::kMicroQR, "MicroQR",
                  code_type, output);
    JudgeCodeType(smartmore::barcode::BarcodeType::kAztec, "Aztec", code_type,
                  output);
    JudgeCodeType(smartmore::barcode::BarcodeType::kMaxicode, "Maxicode",
                  code_type, output);
    JudgeCodeType(smartmore::barcode::BarcodeType::kCodabar, "Codabar",
                  code_type, output);
...
void PrintResult(const smartmore::barcode::BarcodeInfo &result)
{
    ...
    std::cout << " status : " << result.succeed << result.decode_succeed
              << result.detect_succeed << ", ";
    ...
    if (IsIntersect(result.type, smartmore::barcode::BarcodeType::kCode1D))
    {
        ...
        if (overall_grade >= 0)
        {
            std::cout << "ISO15416: {overall: " << overall_grade
                      ...
                      << ", decodability: "
                      << result.iso15416_quality.decodability << "}"
                      << std::endl;

            std::cout << "ISO15416_value: {"
                      << "Rmin: " << std::setprecision(precision)
                      << result.iso15416_quality.rmin_rmax_raw
                      << ", Ecmin: " << std::setprecision(precision)
                      << result.iso15416_quality.ecmin_raw
                      << ", Sc: " << std::setprecision(precision)
                      << result.iso15416_quality.sc_raw
                      << ", Defects: " << std::setprecision(precision)
                      << result.iso15416_quality.defects_raw
                      << ", Decodability: " << std::setprecision(precision)
                      << result.iso15416_quality.decodability_raw << "}"
                      << std::endl;
        }
    }
    else if (IsIntersect(result.type, smartmore::barcode::BarcodeType::kCode2D))
    {
        if (result.iso15415_quality.decode > 0)
        {
            std::cout << "ISO15415: {"
        ...
        if (result.iso29158_quality.decode > 0)
        {
            std::cout << "ISO29158: {"
...
{
    auto filter_boundary = [&](const smartmore::barcode::BarcodeResponse &res)
    {
        // boundary check for image
        std::vector<smartmore::barcode::BarcodeInfo> res_results;
        for (int res_idx = 0; res_idx < res.results.size(); ++res_idx)
        {
            std::array<cv::Point2f, 4> coarse_loc =
                res.results[res_idx].coarse_locate_corner_boxes;
            ...
            if (x_min < 0 || x_max > img_width || y_min < 0 ||
                y_max > img_height)
            {
                continue;
            }
            res_results.emplace_back(res.results[res_idx]);
        }
        return res_results;
    };

    std::vector<smartmore::barcode::BarcodeInfo> res_1_results =
        filter_boundary(res_1);
    std::vector<smartmore::barcode::BarcodeInfo> res_2_results =
        filter_boundary(res_2);

    // std::cout << res_1_results.size() << ", " << res_2_results.size()
    //           << std::endl;
    // std::cout << "res1: ";
    // for (auto &res : res_1_results)
    // {
    //     for (auto &pt : res.coarse_locate_corner_boxes)
    //     {
    //         std::cout << pt << ", ";
    //     }
    //     std::cout << std::endl;
    ...
    for (int res_idx = 0; res_idx < res_1_results.size(); ++res_idx)
    {
        if (res_1_results[res_idx].succeed ^ res_2_results[res_idx].succeed)
        {
        ...
        if (res_1_results[res_idx].type != res_2_results[res_idx].type)
        {
            std::cerr << "The exception of different type using roi! "
                      << res_idx << ": " << res_1_results[res_idx].type << "? "
                      << res_2_results[res_idx].type << std::endl;
            return false;
        }
        if (!DiffResult(res_1_results[res_idx], res_2_results[res_idx]))
        {
...
smartmore::barcode::BarcodeType StringToType(std::string type_string)
{
    ...
    string_to_format_map["dm"] = smartmore::barcode::BarcodeType::kDmcode;
    string_to_format_map["MQR"] = smartmore::barcode::BarcodeType::kMicroQR;
    string_to_format_map["mqr"] = smartmore::barcode::BarcodeType::kMicroQR;
    ...
    string_to_format_map["Vericode"] =
        smartmore::barcode::BarcodeType::kVericode;
    string_to_format_map["Aztec"] = smartmore::barcode::BarcodeType::kAztec;
    string_to_format_map["Maxicode"] =
        smartmore::barcode::BarcodeType::kMaxicode;
    string_to_format_map["Codabar"] =
...
// uninit vs600 npu
void NovaicNetworkUninit() { novaic::CoManager::NetworkUninit(); }

// release vs600 npu mem
void NovaicCommomUninit() { novaic::CoManager::CommomUninit(); }

// init vs600 npu mem
void NovaicInitComMem() { novaic::InitComMem(); }
