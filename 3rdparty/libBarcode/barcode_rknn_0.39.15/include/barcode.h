 ...
 * @brief
 * @version 0.39.15_trackonomy_rknn
 * @date 2025-06-06
 *
...
struct BarcodeParams2DQuota
{
    ...
    std::vector<float> rm_thres = {0.50f, 0.40f, 0.30f, 0.20};
    ...
    bool print_growth_enable = false;
...
struct BarcodeParams1DQuota
{
    ...
    std::vector<float> mod_thres = {0.70f, 0.60f, 0.50f, 0.40f};
...
enum class BarcodeType : uint32_t
{
    ...
    kCode39 = 1 << 0,  // Code 39 1D format.
    kCode93 = 1 << 1,  // Code 93 1D format.
    kCode128 = 1 << 2, // Code 128 1D format.
    kEan8 = 1 << 3,    // EAN-8 1D format.
    kEan13 = 1 << 4,   // EAN-13 1D format.
    kItf = 1 << 5,     // ITF (Interleaved Two of Five) 1D format.
    kUpcA = 1 << 6,    // UPC-A 1D format.
    kUpcE = 1 << 7,    // UPC-E 1D format.
    kUpcEanExtension = 1 << 8,
    ...
    kCode1D = smartmore::barcode::BarcodeType::kCode39 |
              ...
              smartmore::barcode::BarcodeType::kCodabar,
    kCode2D = BarcodeType::kQrcode | BarcodeType::kDmcode |
              BarcodeType::kPdf417 | BarcodeType::kVericode |
              BarcodeType::kMicroQR | BarcodeType::kAztec |
              BarcodeType::kMaxicode,
};
...
{
    kAuto = 0,
    kEfficiency = 1 << 0,  // detection speed first
    kPerformance = 1 << 1, // detection quality first
    kSport = 1 << 2        // sport mode for fast moving scene
};
...
/**
 * @brief struct ScriptResultFilterOp
 * @struct ScriptResultFilterOp
 */
struct ScriptResultFilterOp
{
    std::string regular_expr; ///< regular expressions
    BarcodeType
        type; ///< binding barcode type. if kUnknown, set string filter for all.

    ScriptResultFilterOp() : regular_expr(""), type(BarcodeType::kUnknown) {}
    ScriptResultFilterOp(BarcodeType tp, const std::string &exp)
        : regular_expr(exp), type(tp)
    {
    }
};

/**
 * @brief struct BarcodeSettingParam
 * @struct BarcodeSettingParam
 */
struct BarcodeSettingParams
{
    bool enable; ///< Barcode Setting Mode enable
    BarcodeType
        type; ///< Barcode Setting code type, support 2d code, ex. QR | DM
    std::string regular_expr; ///< Barcode Setting regular expressions

    BarcodeSettingParams()
        : enable(false), type(BarcodeType::kUnknown), regular_expr("")
    {
    }
    BarcodeSettingParams(bool enable, BarcodeType type,
                         std::string regular_expr)
        : enable(enable), type(type), regular_expr(regular_expr)
    {
    }
};
...
struct ISO15416Quality
{
    ...
    ISO15416Quality()
        : rmin(-1), ecmin(-1), sc(-1), mod(-1), defects(-1), decodability(-1),
          overall(-1), rmin_raw(0), rmax_raw(0), rmin_rmax_raw(0), sc_raw(0),
          ecmin_raw(0), mod_raw(0), defects_raw(0), decodability_raw(0)
    {
    }
};
...
struct ISO15415Quality
{
    ...
    float rm_value;    ///< Reflectance Margin value
    ...
    ISO15415Quality()
        : overall(-1), decode(-1), sc(-1), mod(-1), rm(-1), an(-1), gn(-1),
          uec(-1), fpd(-1), r_min_value(-1), r_max_value(-1), gt_value(-1),
          sc_value(-1), mod_value(-1), rm_value(-1), an_value(-1), gn_value(-1),
          uec_value(-1), fpd_value(-1)
    {
    }
};
...
struct ISO29158Quality
{
    ...
    ISO29158Quality()
        : overall(-1), decode(-1), cc(-1), cm(-1), rm(-1), r_target(-1),
          ddg(-1), an(-1), gn(-1), uec(-1), cc_value(-1), cm_value(-1),
          rm_value(-1), r_target_value(-1), an_value(-1), gn_value(-1),
          uec_value(-1), ddg_value(-1)
    {
    }
};
...
struct BarcodeInfo
{
    ...
    bool is_setting_code; ///< barcode setting status
    ...
    int mask_pattern = -1;     ///< for QR only, 0~7
    ...
    int group_idx;                  /// each dl coarse locate result group idx
    std::array<cv::Point2f, 4>
        precise_locate_corner_boxes; ///< four dl precise locate
                                     ///< corner_boxes (only dl)
    std::array<cv::Point2f, 4>
        package_boxes;                ///< four dl package loacate
                                      ///< corner_boxes (only dl)
    std::array<int, 2> incomplete_num{0}; ///< dm incomplete num
    ...
    // iso15416
    float unit_width_ratio; // width ratio of unit bright and dark module for
                            // two width
    std::string bit_str;    // code bit string
    cv::Mat one_decode_image;
    float cols_scale;

    int defect_status;    ///< defect status [0000, 1111];
    bool is_dl_processed; ///< dl filter process flag

    BarcodeInfo()
        : idx(0), is_setting_code(false), detect_succeed(false),
          decode_succeed(false), succeed(false), type(BarcodeType::kUnknown),
          result(""), ppm(0), mirror(MirrorType::kAuto),
          polarity(Polarity::kAuto), barcode_angle_deg(0.f), version({0, 0}),
          decode_time(0.f), raw(""), grid_pts{}, unit_width_ratio(0.0),
          bit_str(""), one_decode_image(cv::Mat()), cols_scale(1.0f),
          defect_status(15), is_dl_processed(false)
    {
    }
};
 ...
 *
 * auto_upca                 auto transfer ean13 to upca
 *
 * codabar_enable_check_digit    params codabar check enable
 */
class Barcode
{
public:
     ...
     * auto_upca                    |false             |bool
     * codabar_enable_check_digit    |false             |bool
     *
     ...
     * @return run status, 1 success 0 fail
     */
    bool RunDetect(const BarcodeRequest &input, BarcodeResponse &output);
    ...
    /**
     * @brief Set running mode for barcode
     * @param running_mode RunningMode::kxx
     * @todo implementation
     */
    void SetRunningMode1D(RunningMode running_mode);
    ...
    /**
     * @brief Get running mode for barcode
     * @return RunningMode
     * @todo implementation
     */
    RunningMode GetRunningMode1D() const;
    ...
    /**
     * @brief set dm defect enable signal
     *
     * @param value  bool
     */
    void SetDmDefectEnable(bool value);

    /**
     * @brief get dm defect enable signal
     *
     * @return bool
     */
    bool GetDmDefectEnable() const;

    /**
     * @brief set dm priori defect enable signal
     *
     * @param value  bool
     */
    void SetDmPrioriDefectEnable(bool value);

    /**
     * @brief get dm priori defect enable signal
     *
     * @return bool
     */
    bool GetDmPrioriDefectEnable() const;
    ...
    /**
     * @brief set codabar check digit enable signal
     * @param value
     */
    void SetCheckDigitEnableCodabar(bool value);

    /**
     * @brief get codabar check digit enable signal
     * @return bool
     */
    bool GetCheckDigitEnableCodabar(void) const;

    /**
     * @brief set auto upca signal
     * @param value
     */
    void SetAutoUpca(bool value);

    /**
     * @brief get auto upca signal
     * @return bool
     */
    bool GetAutoUpca(void) const;
    ...
    /**
     * @brief set package detect enable
     * @param value bool
     */
    void SetPackageDetectEnable(bool value);

    /**
     * @brief get package detect enable
     * @return bool
     */
    bool GetPackageDetectEnable(void) const;
    ...
    /**
     * @brief set quota enable and threshold in 2d
     * @param enable, enable smart quality, expect grade
     */
    void SetQualityStrategy(bool enable, int grade);
    ...
    /**
     * @brief set script result filter options
     * delimiter: #@&-%$Seperator&@#
     * @param std::vector<ScriptResultFilterOp>
     */
    void SetScriptResultFilterOps(const std::vector<ScriptResultFilterOp> &ops);

    /**
     * @brief get script result filter options
     * @return std::vector<ScriptResultFilterOp>
     */
    std::vector<ScriptResultFilterOp> &GetScriptResultFilterOps(void) const;
    ...
    /**
     * @brief set vericode password
     */
    void SetVericodePassword(const std::string &password);

    /**
     * @brief set vericode enable
     */
    void SetVericodeEnable(const bool enable_vericode);

    /**
     * @brief set Handheld Mode and Center point
     * @param handheld_mode HandheldModeType,for example kUnSet, kFull,
     * kCenter
     * @param handheld_center cv::Point2f, Handheld Center coordinates of
     * Aiming light
     * @return void
     */
     ...
     * @param handheld_center_wh In HandheldMode KCenterROI, Width and Height of
     * CenterROI Box
     * @return void
     */
    void SetHandheldCenterWH(const std::array<int, 2> &handheld_center_wh);
    ...
    /**
     * @brief Set GS1 decode status
     *
     * @param val default false
     */
    void SetGs1Enable(bool val);

    /**
     * @brief Get GS1 decode status
     *
     * @return bool
     */
    bool GetGs1Enable(void) const;

    /**
     * @brief Set GTIN decode status
     *
     * @param val default false
     */
    void SetGtinEnable(bool val);
    /**
     * @brief Get GTIN decode status
     *
     * @return true
     * @return false
     */
    bool GetGtinEnable(void) const;

    /**
     * @brief Set UDI decode status
     *
     * @param val default false
     */
    void SetUdiEnable(bool val);
    /**
     * @brief Get UDI decode status
     *
     * @return true
     * @return false
     */
    bool GetUdiEnable(void) const;

    /**
     * @brief Set Medicine mode status
     *
     * @param val default false
     */
    void SetMedicineEnable(bool val);
    /**
     * @brief Get Medicine mode status
     *
     * @return true
     * @return false
     */
    bool GetMedicineEnable(void) const;

    /**
     * @brief Set barcode setting parameters
     *
     * @param param
     */
    void SetBarcodeSettingParams(const BarcodeSettingParams &params);

    /**
     * @brief Get barcode setting parameters
     *
     * @return BarcodeSettingParams
     */
    BarcodeSettingParams &GetBarcodeSettingParams(void) const;

    /**
     * @brief Set Online Learning Enable
     *
     * @param val default false
     */
    void SetOnlineLearningEnable(bool val);

    /**
     * @brief Get Online Learning Status
     *
     * @return true
     * @return false
     */
    bool GetOnlineLearningEnable(void) const;
