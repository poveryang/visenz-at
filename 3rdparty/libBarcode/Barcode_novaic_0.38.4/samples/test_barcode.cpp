...
int main(int argc, char **argv)
{
    ...
    bool is_updata_novaic_model_info = false;
    ...
    std::string help_info = R"""(
        -d directory path
        -t barcode type, can be [1D, 2D, CODE_39, CODE_93, CODE_128, EAN_8, EAN_13, ITF, UPC_A, UPC_E, UPC_EAN_EXTENSION, QR, DM, PHARMA_CODE, Pdf417, Vericode, MQR, Codabar], multi-type input is supported
        ...
        -U (optional) updata novaic model message
        -h help
    )""";
...
#else
    while (-1 != (ret = getopt(argc, argv, "d:t:n:m:c:e:Mo:l:rf:s:i:wh:U")))
#endif
    {
        switch (ret)
        {
        ...
        case 'n':
            ...
            break;
        case 'm':
            mode = static_cast<smartmore::barcode::RunningMode>(atoi(optarg));
            break;
        case 'c':
        ...
        case 'w':
            ...
            break;
        case 'U':
            is_updata_novaic_model_info = true;
            break;
        default:
    ...
    std::cout << argv[0] << " -d " << directory_path << " -t " << barcode_type
              ...
              << " -U " << is_updata_novaic_model_info << std::endl;
    ...
    std::string model_info_path =
        "./3rdparty/novaic_model/vs800_52x/novaic_model_info.json";
    if ("novaic100w" == signature)
    {
        model_info_path =
            "./3rdparty/novaic_model/vs600_56x/novaic_model_info.json";
    }
    if (!is_updata_novaic_model_info)
    {
        novaic::InitComMem(model_info_path, signature);
    }
#endif
    ...
    if (is_updata_novaic_model_info)
    {
        novaic::NovaicNetWork::WriteModelParamsToJsonFile(model_info_path,
                                                          signature);
        NovaicInitComMem();
    }
#endif
    ...
    // sdk.SetGs1Enable(true);
    // sdk.SetDmDefectEnable(true);
    // sdk.SetDmECCType(smartmore::barcode::DMECCType::kAuto);
    // sdk.SetTimeoutPeriod(1000);
    // sdk.SetScriptFilterString("((?:[\\s\\S]{14}))$");

    // smartmore::barcode::BarcodeSettingParams params(
    //     true, smartmore::barcode::BarcodeType::kDmcode, "^@.*");
    // sdk.SetBarcodeSettingParams(params);
    ...
    while (loop_counts != 0)
    {
        ...
        for (int i = 0; i < file_list.size(); i++)
        {
            ...
            for (auto &result : output.results)
            {
                ...
                cv::putText(draw, "idx:" + std::to_string(coarseloc_idx),
                            result.coarse_locate_corner_boxes[0],
                ...
                if (result.detect_succeed)
                {
                    ...
                    if (result.succeed)
                    {
                        ...
                        cv::putText(
                            draw, result.result, result.center,
                            cv::FONT_HERSHEY_PLAIN, 2.0, cv::Scalar(0, 255, 0));
                        std::vector<cv::Point> pts = {
                    ...
                    {
                        cv::polylines(draw, pts, true, cv::Scalar(0, 0, 255));
                    }
                }
            }

            cv::putText(draw, "coarseloc_num: " + std::to_string(coarseloc_idx) + " " +
    ...
    sdk.CloseNetWork();
    NovaicCommomUninit();
    novaic::NovaicNetWork::CompareWithFile(model_info_path, signature);
#endif
