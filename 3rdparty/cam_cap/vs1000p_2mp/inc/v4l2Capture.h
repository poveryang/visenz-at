...
#define v4l2_dbg(fmt, args...)   \
		v4l2_printf(DBG_LEVEL, "[dbg] " fmt, ##args)
#define v4l2_info(fmt, args...)  \
		v4l2_printf(INFO_LEVEL, "[info] " fmt, ##args)
#define v4l2_err(fmt, args...)   \
	    v4l2_printf(ERR_LEVEL, "[err] " fmt, ##args)
#else
...
#define NUM_PLANES 1
#define SENSOR_INFO_FILE "/proc/sensor_info"
...
#define V4L2_CID_SMORE_BASE              (V4L2_CID_USER_BASE + 0x10a0)
#define V4L2_CID_SMORE_SET_TRIGGER_PLUSE (V4L2_CID_SMORE_BASE + 3)
#define V4L2_CID_SMORE_SET_CAPTURE_MODE  (V4L2_CID_SMORE_BASE + 4)
#define V4L2_CID_SMORE_SET_MASTER_STREAM (V4L2_CID_SMORE_BASE + 5)
#define V4L2_CID_SMORE_SET_LIGHT_MODE    (V4L2_CID_SMORE_BASE + 6)
...
public:
	...
	int setLightState(int enable);
    int setLightMode(E_LIGHT_MODE mode);
...
private:
	...
	//V4L2_MEMORY_MMAP使用v4l2框架的拷贝  V4L2_MEMORY_USERPTR是使用sdma拷贝
